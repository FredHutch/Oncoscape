    ...
    function heatmap() {
        ...
        function HeatmapController(d3, osApi, osCohortService, $state, $timeout, $scope, $stateParams, $window) {
            ...
            vm.draw = function(){
                ...
                heatmap(colmap, data.x.matrix, 
                        hmWidth, 
                        hmHeight, 
                        (vm.rowDendrogram ? 80 : 0)+layout.left, 
                        (vm.colDendrogram ? 80 : 0)+ 100);

                dendrogram(rowDend, data.x.rows,    
                    80, hmHeight, 
                    layout.left, (vm.colDendrogram ? 180 : 100) , false);

                dendrogram(colDend, data.x.cols,    
                    hmWidth, 80, 
                    (vm.rowDendrogram ? 80 : 0)+layout.left, 100, true);

                axis(xaxis, 
                    data.x.matrix.rows,     
                    160, hmHeight, hmWidth + (vm.rowDendrogram ? 80 : 0)+layout.left, (vm.colDendrogram ? 180 : 100), false);

                axis(yaxis, data.x.matrix.cols,
                    hmWidth, 160, (vm.rowDendrogram ? 80 : 0)+layout.left, hmHeight + (vm.colDendrogram ? 180 : 100), true);
