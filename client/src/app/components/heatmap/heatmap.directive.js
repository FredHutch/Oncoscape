        ...
        function HeatmapController(d3, osApi, osCohortService, $state, $timeout, $scope, $stateParams, $window) {
            ...
            vm.load = function(){
                ...
                osApi.query("brca_psi_bradleylab_miso", {
                    ...
                    d.scale = vm.scale.name.toLowerCase();
                    d.krow = vm.rowGroups;
                    d.kcol = vm.colGroups;
            ...
            function dendrogram(svg, data, width, height, x, y, rotated) {
               ...
               var calcHeight = function(data, height) {
                    height += data.height;
                    if (data.children){
                        return calcHeight(data.children[0], height)
                    }else{
                        return height;
                    }
                };
...
//                    d3.select(this).transition().call(d3.event.target.move, d1.map(x));
            ...
            vm.draw = function(){
                var layout = osApi.getLayout();
                var width = $window.innerWidth - layout.left - layout.right - 20;
                ...
                heatmap(colmap, data.matrix, 
                        ...
                        (vm.colDendrogram ? 80 : 0)+ 70);

                dendrogram(rowDend, data.rows,    
                    80, hmHeight, 
                    layout.left, (vm.colDendrogram ? 150 : 70) , false);

                dendrogram(colDend, data.cols,    
                    hmWidth, 80, 
                    (vm.rowDendrogram ? 80 : 0)+layout.left, 70, true);

                axis(xaxis, 
                    data.matrix.rows,     
                    160, hmHeight, hmWidth + (vm.rowDendrogram ? 80 : 0)+layout.left, (vm.colDendrogram ? 150 : 70), false);

                axis(yaxis, data.matrix.cols,
                    hmWidth, 160, (vm.rowDendrogram ? 80 : 0)+layout.left, hmHeight + (vm.colDendrogram ? 150 : 70), true);
