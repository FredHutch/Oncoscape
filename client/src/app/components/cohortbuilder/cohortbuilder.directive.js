    ...
    function cohortBuilder() {
        ...
        function CohortBuilderController(osApi, $state, $timeout, $scope, $stateParams, _) {
            ...
            osApi.query("_collections").then(function(result) {
                ...
                vm.dataSources =
                    _.chain(data).pluck("source").uniq().map(function(v) {
                        return {
                            name: v,
                            diseases: _.chain(data).where({
                                source: v
                            }).pluck("disease").uniq().map(function(v) {
                                return {
                                    name: v,
                                    tables: _.chain(data).where({
                                        source: "tcga",
                                        disease: v
                                    }).map(function(v) {
                                        return {
                ...
                $scope.$watch("vm.dataTable", function() {
                    console.log(osApi.queryString("_stats", {
                        collection: vm.dataTable.collection
                    }));
                    osApi.query("_stats", {
                        collection: vm.dataTable.collection
