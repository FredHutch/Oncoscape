...
function request(req, cb) {
    var query = "http://localhost:80/api/" + req.table;
    if (req.query) query += "/?q=" + encodeURIComponent(JSON.stringify(req.query));
    load(query, cb);
}
...
var processGenes = (function() {
    ...
    return function(options) {
        ...
        // Load Genes If Options Change
        return new Promise(function(resolve, reject) {
            if (_options.layout != options.layout) {
                request({
                    'table': 'mp_genesets',
                    query: {
                        name: options.layout
                    ...
                    _genes = JSON.parse(response.responseText)[0].genes
                    console.log("Genes : "+_genes.length);
                    _genes = _genes
                    	.filter(function(item){
                    		// Remove Genes That Are Not Positioned On Chromosome
                    		return (item.position.x!=0 && item.position.y!=0)
                    	})
                    	.map(function(item) {
	                        var node = {
	                            group: "nodes",
...
var processPatients = (function() {
    ...
    return function(options) {
        ...
        var update = function() {
            ...
            // Process Colors
            if (options.colors.name != _options.colors.name) {
                degmap = degmap || {};

                switch (options.colors.name.toLowerCase()) {
                    case "default":
                        _patients.forEach(function(patient) {
                            var data = patient.data;
                            this[data.id] = this[data.id] || {};
                            this[data.id].color = "#1396de";
                        }, degmap);
                        break;
                    case "age":
                        _patients.forEach(function(patient) {
                            var data = patient.data;
                            this[data.id] = this[data.id] || {};
                            this[data.id].color = "rgb(19, 150, 222)";
                        }, degmap);
                        break;
                    case "gender":
                        _patients.forEach(function(patient) {
                            var data = patient.data;
                            this[data.id] = this[data.id] || {};
                            this[data.id].color = (data.id.indexOf("02") > 2) ? "rgb(0, 0, 255)" : "rgb(255, 0, 255)";
                        }, degmap);
                        break;
                }
            }
        ...
        return new Promise(function(resolve, reject) {
            ...
            // Load Patients If Options Change
            if (_options.layout != options.layout) {

                request({
                    table: options.data
                }, function(response) {
                    var patientData = JSON.parse(response.responseText);

                    request({
                        table: options.layout
                    }, function(response) {
                        _patients = JSON.parse(response.responseText);
                        console.log("Patients : "+_patients.length);
                        _patients = _patients
                        	.map(function(item) {
	                            // Help Ids Are not Lining Up
	                            var data = patientData.filter(function(pat) {
	                            	var pid1 = pat.patient_ID.replace(/\./g, '-');
	                            	var pid2 = this.toString();
	                            	pid2 = pid2.substring(0,pid2.lastIndexOf("-"))
	                                return (pid1==pid2);
	                            }, item.name);
	                            ...
	                            if (data.length==1){
									...
									data.nodeType = "patient",
									data.degree = 1,
									data.sizeBdr = 300,
	                                data.sizeEle = 500,
	                                data.sizeLbl = 500,
	                                data.subType = "unassigned",
	                                data.hasPatientData = true
	                            }else{
	                            	data = {
	                        	...
	                        	return v.data.hasPatientData;
	                        });
						console.log("Patients : "+_patients.length);

                        send("patients_delete");
                        send("patients_insert", _patients);
                        update();
                        resolve(_patients);
                    });
                });
...
var processEdges = (function() {
    ...
    return function(options, genes, patients) {


    	var update = function(){
    		
    		
    		if (options.patients.selected.length==0 && options.genes.selected.length==0){
    			send("edges_delete");
    			_options = options;
    			return;
    		}
        ...
        // Load Edges If Options Change
        if (_options.edges.layout != options.edges.layout) {
        	console.log(options.edges.layout)
            request({
                'table': options.edges.layout
            }, function(response) {
            	
                 _edges = JSON.parse(response.responseText);
                 ...
                 _edges = _edges
					.filter(function(item){  // Remove Edges w/ Invalid Gene
						for (var i=0;i<this.length; i++){ if (item.g==this[i].data.id) return true; }
						return false;
					}, genes)
					.filter(function(item){  // Remove Edges w/ Invalid Gene
						for (var i=0;i<this.length; i++){ if (item.p==this[i].data.id) return true; }
						return false;
					}, patients);

					console.log("Edges : "+ _edges.length);

                _edges = _edges.map(function(item) {
                    var edge = {
                        group: "edges",
                        grabbable: false,
                        locked: true,
...
self.addEventListener('message', function(msg) {
    ...
    switch (msg.cmd) {
        case "setOptions":
            processGenes(msg.data.genes).then(function(_genes) {
                
                processPatients(msg.data.patients).then(function(_patients){

                	processEdges(msg.data, _genes, _patients);
