(function() {
    ...
    angular
        .module('oncoscape')
        .directive('osThumbnailMenu', thumbnailMenu);
    ...
    function thumbnailMenu() {
        ...
        var directive = {
            restrict: 'E',
            templateUrl: 'app/components/thumbnailmenu/thumbnailmenu.html',
            controller: ThumbnailMenuController,
            controllerAs: 'vm',
            scope: {},
        ...
        /** @ngInject */
        function ThumbnailMenuController(osApi, $state, $scope, $sce, $timeout, $rootScope, $filter, d3) {
            ...
            // Histogram 
            var histSvg = d3.select("#thumbnailmenu-chart").append("svg")
                .attr("width", 260)
                .attr("height", 150)
                .append("g");
            var histSingleValueLabel = angular.element("#thumbnailmenu-single-value");
            ...
            $scope.$watch('vm.cohortFeature', function() {
                ...
                if (data.type == "factor") {
                    if (data.hist.length == 1) {
                        histSingleValueLabel.text(data.hist[0].label).css("display", "block").removeClass("thumbnailmenu-single-value-numeric");
                        histSvg.classed("cohort-chart-hide", true);
                        return;
                    }
                } else {
                    if (data.min == data.max) {
                        histSingleValueLabel.text(data.min).css("display", "block").addClass("thumbnailmenu-single-value-numeric");
                        histSvg.classed("cohort-chart-hide", true);
                        return;
                    }
                }
                ...
                var yScale = d3.scaleLinear()
                    ...
                    .range([0, 135]);
                var bars = histSvg
                    .selectAll(".thumbnail-menu-chart-bar")
                    .data(data.hist);
                bars.enter()
                    .append("rect")
                    .attr("class", "thumbnail-menu-chart-bar")
            ...
            // Survival
            var surSvg = d3.select("#thumbnailmenu-survival").append("svg");
            var surLines = surSvg.append("g")
                .selectAll("thumbnailmenu-survival-percent-line")
                .data([0.25, 0.5, 0.75]);

            surLines.enter()
                .append("line").attr("class", "thumbnailmenu-survival-percent-line")
                .attr("stroke-width", 1)
                .attr("stroke", "#EAEAEA")
