...
from json import *
ws = create_connection("ws://localhost:9013")
#------------------------------------------------------------------------------------------------------------------------
def runTests():
...
def testEcho():
  ...
  assert(result["payload"][0] == 'hello from PLSR/inst/ws/wsWrapper.py: python')
...
#------------------------------------------------------------------------------------------------------------------------
interactive = (sys.argv[0] != "testPLSRWebSocketOperations.py")
if(not(interactive)):
  runTests()
  print("all testPLSRWebSocketOperations.py tests passed")
