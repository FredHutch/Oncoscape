...
var pi3kAktModule = (function () {
  ...
  var layoutMenu;
  var thisModulesName = "EGFR Pathway"; // "PI3K-AKT Pathway";
  var thisModulesOutermostDiv = "pi3kAktPathwayDiv";
...
function initializeUI ()
{
  ...
  for(var i=0;i< operations.length; i++){
   ...
   searchBox = $("#pi3kAktSearchBox");

   edgeTypeSelector = $("#pi3kAktPathwayEdgeTypeSelector");
   mouseOverReadout = $("#pi3kAktMouseOverReadout");
   loadNetwork();
...
function loadNetwork ()
{
  ...
  //console.log("loadnetwork, node count: " + pi3kAktNetwork.elements.nodes.length);
  cyPathway = $("#cyPathwayDiv");
  cyPathway.cytoscape({
     //elements: pi3kAktNetwork.elements,
     //style: pi3kAktVizmap[0].style,
...
function displayPathway(msg)
{
   ...
   if(msg.status == "success"){
      ...
      cyPathway.resize();
      cyPathway.fit(50);
      console.log("concluding displayPathway pi3kakt");
      cyPathway.edges().show();
...
function datasetSpecified (msg)
{
   ...
   var pathway = "egfr.json.RData";
   if($.inArray(pathway, variableNames) >= 0){
      var newMsg = {cmd: "getPathway",  callback: "pi3kAktDisplayPathway",
                    status: "request", payload: "g.egfr.json"};
      hub.send(JSON.stringify(newMsg));
...
{
  console.log("posting new status to egfr status div: " + msg);
  statusDiv.text(msg);
...
function gbmLggDzSpecificTests()
{
  ...
  QUnit.test("markersAndSamples: search and select EGFR", function(assert){
     ...
     var targetNode = "EGFR";
     $("#pi3kAktSearchBox").val(targetNode);
     var e = jQuery.Event("keydown");
     e.which = 13;  
     $("#pi3kAktSearchBox").trigger(e);
...
//----------------------------------------------------------------------------------------------------
 return{
     init: function(){
        ...
        hub.addMessageHandler("sendSelectionTo_EGFR Pathway", handleIncomingIdentifiers);
        hub.registerSelectionDestination(selectionDestinations, thisModulesOutermostDiv);
        hub.addMessageHandler("datasetSpecified", datasetSpecified);
        hub.addMessageHandler("pi3kAktDisplayPathway", displayPathway);
