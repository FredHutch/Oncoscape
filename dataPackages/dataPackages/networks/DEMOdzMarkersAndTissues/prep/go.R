...
#------------------------------------------------------------------------------------------------------------------------
if(!exists("mtx.mut")){
  load("../../../DEMOdz/inst/extdata/mtx.mut.RData")
  rownames(mtx.mut) <- sub("\\.01$", "", rownames(mtx.mut))
  }
if(!exists("mtx.cn")){
   load("../../../DEMOdz/inst/extdata/mtx.cn.RData")
...
export <- function(rcy)
{
   ...
   print("optionally: cp markers.json.RData ../../../DEMOdz/inst/extdata/")
...
createMutationGraph <- function(tbl, orphan.patients)
{
  ...
  nodeData(g, genes, "tooltip") <- paste("gene ", genes)
  nodeData(g, patients, "tooltip") <- paste("pt. ", patients)

  edgeData(g, patients, genes, "mutation") <- mutations
  edgeData(g, patients, genes, "tooltip") <- paste(patients, " -> ", genes, " (", mutations, ")", sep="")
...
createCopyNumberGraph <- function(tbl)
{
  ...
  patients.sub <- patients[cnLoss.1]
  genes.sub    <- genes[cnLoss.1]
  edgeData(g, patients.sub, genes.sub, "tooltip") <- paste(patients.sub, " (cn-1) ", genes.sub, sep="")
  
  patients.sub <- patients[cnLoss.2]
  genes.sub    <- genes[cnLoss.2]
  edgeData(g, patients.sub, genes.sub, "tooltip") <- paste(patients.sub, " (cn-2) ", genes.sub, sep="")
  
  patients.sub <- patients[cnGain.1]
  genes.sub    <- genes[cnGain.1]
  edgeData(g, patients.sub, genes.sub, "tooltip") <- paste(patients.sub, " (cn+1) ", genes.sub, sep="")
  
  patients.sub <- patients[cnGain.2]
  genes.sub    <- genes[cnGain.2]
  edgeData(g, patients.sub, genes.sub, "tooltip") <- paste(patients.sub, " (cn+2) ", genes.sub, sep="")
...
createChromosomeGraph <- function(tbl)
{
  ...
  tooltips <- paste(genes, " on ", chromosomes, sep="")
  edgeData(g, genes, chromosomes, "tooltip") <- tooltips
...
#----------------------------------------------------------------------------------------------------
createBaseGraph <- function()
{
   all.nodes <- sort(unique(c(rownames(mtx.mut),
                              rownames(mtx.cn),
                              colnames(mtx.mut),
                              colnames(mtx.cn))))
   ...
   indices <- match(patients, rownames(tbl.gbmDzSubTypes))
   dzSubTypes <- as.character(tbl.gbmDzSubTypes$gbmDzSubType[indices])
   dzSubTypes <- sub("NA", "unknown", dzSubTypes)
   tooltip.patients <- paste(patients, " (", dzSubTypes, ")", sep="")
   
   nodeData(g, patients,    attr="tooltip") <- tooltip.patients
   nodeData(g, genes,       attr="tooltip") <- genes;
   nodeData(g, chromosomes, attr="tooltip") <- chromosomes;
