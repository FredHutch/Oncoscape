...
#      choose lusc, #cases with data (528 on 2/10/15)
#      choose "Biotab"
#      click "build archive" 
...
#
#      biospecimen_aliquot_lusc.txt
#      biospecimen_analyte_lusc.txt
#      biospecimen_cqcf_lusc.txt                 cqcf: case quality control form
#      biospecimen_diagnostic_slides_lusc.txt
#      biospecimen_normal_control_lusc.txt
#      biospecimen_portion_lusc.txt
#      biospecimen_protocol_lusc.txt
#      biospecimen_sample_lusc.txt
#      biospecimen_shipment_portion_lusc.txt
#      biospecimen_slide_lusc.txt
#      biospecimen_tumor_sample_lusc.txt
#      clinical_drug_lusc.txt
#      clinical_follow_up_v1.0_lusc.txt
#      clinical_follow_up_v1.0_nte_lusc.txt
#      clinical_nte_lusc.txt
#      clinical_omf_v4.0_lusc.txt
#      clinical_patient_lusc.txt
#      clinical_radiation_lusc.txt
#
...
#   clinical_radiation_lusc.txt
#   clinical_follow_up_v1.0_lusc.txt
#   clinical_follow_up_v1.0_nte_lusc.txt      nte: new tumor event
#   clinical_nte_lusc.txt
#   clinical_omf_v4.0_lusc.txt                omf: other malignancy form
#
#------------------------------------------------------------------------------------------------------------------------
...
currDir <- getwd()
setwd("../../../../RawData/TCGAlusc/Clinical_5-18-15/")

study="TCGAlusc"

tbl.pt <- read.table("clinical_patient_lusc.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)#√
...
tbl.drug <- read.table("clinical_drug_lusc.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)#√
tbl.drug <- tbl.drug[3:nrow(tbl.drug),]
tbl.rad <- read.table("clinical_radiation_lusc.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)#√
tbl.rad <- tbl.rad[3:nrow(tbl.rad),]
tbl.f1 <- read.table("clinical_follow_up_v1.0_lusc.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)#√
tbl.f1 <- tbl.f1[3:nrow(tbl.f1),]
tbl.nte <- read.table("clinical_nte_lusc.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)#√
...
run <- function()
{
   ...
   checkEquals(length(history), 5235)
   checkEquals(as.list(table(unlist(lapply(history, function(e) e["Name"])))), list(`Absent`=39,`Background`=488, `Birth`=495, `Diagnosis`=495,`Drug`=321,`Encounter`=1333,  `Pathology`=570, `Procedure`=67, `Progression`=25,  `Radiation`=77, `Status`=495, `Tests`= 830))
...
create.DOB.record <- function(patient.id)
{
   ...
   if( race == "[Not Available]" || race == "[Not Evaluated]" || race == "[Unknown]") race = NA
   if(!is.na(race)) race=tolower(race)

   if( ethnicity == "[Not Available]" || ethnicity == "[Not Evaluated]" || ethnicity == "[Unknown]") ethnicity  = NA
   if(!is.na(ethnicity)) ethnicity=tolower(ethnicity)
   
   return(list(PatientID=patient.id, PtNum=patient.number, study="TCGAlusc", Name="Birth", Fields= list(date=c(dob), gender=gender, race=race, ethnicity=ethnicity)))
...
test_create.DOB.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.18.4086", PtNum=15, study="TCGAlusc", Name="Birth", Fields=list(date="01/12/1944", gender="male", race=NA, ethnicity=NA)))
    ...
    x <- create.DOB.record("TCGA-63-A5MR") #race == "[Not Evaluated]", diagnosis.year == "[Not Available]"
    checkEquals(x, list(PatientID="TCGA.63.A5MR", PtNum=269, study="TCGAlusc", Name="Birth", Fields=list(date=NA, gender="female", race=NA, ethnicity=NA)))
...
create.Chemo.record <- function(patient.id)
{
   ...
   if(nrow(tbl.drugSub)>0){
     for(chemoEvent in 1:nrow(tbl.drugSub)){
      ...
      if(start.chemoDate !="[Not Available]" && !is.na(diagnosis.date)){
         start.date <- reformatDate(format(diagnosis.date + as.integer(start.chemoDate)))
      } else{ start.date = NA }
      if( end.chemoDate !="[Not Available]"&& !is.na(diagnosis.date)) {
...
# t<-  sapply(tbl.drug$pharmaceutical_therapy_drug_name, function(drug){
      if( drug == "[Not Available]"){ drug = NA}
          ...
          if( drug == "Tipfarnib (R115777)"){ drug ="Tipfarnib (R115777)"}
...
#    $schedule    5/28, every 2 weeks
    ...
    if(route == "Intramuscular (IM)") route <- "IM"
    if(route == "Intravenous (IV)" ) route <- "IV"
    if(route == "Oral" ) route <- "Orally"
...
test_create.Chemo.record <- function()
{
    ...
    x <- create.Chemo.record("TCGA-18-3412")
    checkTrue(is.list(x))
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.18.3412", PtNum=7, study=study, Name="Drug", Fields=list(date=c("02/25/2005","04/28/2005"), therapyType="Chemotherapy", agent="Vinorelbine", intent=NA, dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))

   x <- create.Chemo.record("TCGA-NC-A5HT")  #no start date
...
test_create.Diagnosis.record <- function()
{
   ...
   x <- create.Diagnosis.record(tcga.ids[1])
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.18.3406", PtNum=1, study=study, Name="Diagnosis", Fields=list(date= "01/01/2003", disease="Lung", siteCode="18")))
    x <- create.Diagnosis.record("TCGA-63-A5MI") #diagnosis. year == "[Not Available]"
   checkEquals(x[[1]], list(PatientID="TCGA.63.A5MI", PtNum=263, study=study, Name="Diagnosis", Fields=list(date= NA, disease="Lung", siteCode="63")))
} # test_create.Diagnosis.record
#------------------------------------------------------------------------------------------------------------------------
  ...
  if(length(duplicates)>0){
   ...
   if(nrow(tbl.fu.rows)>0){
 	for(i in 1:nrow(tbl.fu.rows)){
	  ...
	  if(row["new_tumor_event_dx_days_to"] != "[Not Available]" && !is.na(diagnosis.date)){
...
test_create.Progression.record <- function()
{
    ...
    x <- create.Progression.record("TCGA-33-AASB")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "event", "number"))
    checkEquals(x[[1]], list(PatientID="TCGA.33.AASB", PtNum=89, study=study, Name="Progression", Fields=list(date="01/31/2002", event="Distant Metastasis", number=1)))
    x <- create.Progression.record("TCGA-O2-A52Q")
    checkEquals(x[[1]], list(PatientID="TCGA.O2.A52Q", PtNum=490, study=study, Name="Progression", Fields=list(date="03/29/2005", event="Distant Metastasis", number=1)))
} # test_create.Progression.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Progression.records <- function(patient.ids)
{
   ...
   tbl.ptGood  <- subset(tbl.pt,initial_pathologic_dx_year != "[Not Available]")
   tbl.nteSub <- subset(tbl.nte, bcr_patient_barcode %in% tbl.ptGood$bcr_patient_barcode & new_tumor_event_dx_days_to != "[Not Available]")
...
#    $disease     "Lung"
    ...
    for(omfEvent in 1:nrow(tbl.omfSub)){
      ...
      if(atTumorsite == "[Not Available]" | atTumorsite == "[Unknown]") atTumorsite = NA
      ...
      if(omfOffset == "[Not Available]" | omfOffset == "[Pending]" | is.na(diagnosis.date)){ omf.date = NA
...
test_create.Radiation.record <- function()
{
    ...
    x <- create.Radiation.record( "TCGA-18-3407")
    checkTrue(is.list(x))
    checkEquals(x[[1]], list(PatientID="TCGA.18.3407", PtNum=2, study=study, Name="Radiation", Fields=list(date=NA, therapyType="Prior Malignancy", intent=NA, target="Locoregional", totalDose=NA, totalDoseUnits=NA, numFractions=NA)))
    x <- create.Radiation.record("TCGA-46-6026")  # tbl.rad has both dates
    checkEquals(x[[1]], list(PatientID="TCGA.46.6026", PtNum=170, study=study, Name="Radiation", Fields=list(date=c("03/05/2010", "04/20/2010"), therapyType="external beam", intent="adjuvant", target="Primary Tumor Field", totalDose="6,120", totalDoseUnits="cGy", numFractions=34)))
} # test_create.Radiation.record
#------------------------------------------------------------------------------------------------------------------------
...
create.Encounter.record <- function(patient.id)
{
   ...
   for(encEvent in 1:nrow(tbl.encSub)){
      ...
      if(!(is.na(carbon.monoxide.diffusion))){
          ...
          study=study,
          Name=name,
          Fields = list(type=encType, kps=KPS, ecog=ECOG, date=NA,fev1.ratio=NA, fev1.percent=NA,
                   carbon.monoxide.diffusion=NA, systolic=NA, diastolic=NA,height=NA,weight=NA,BSA=NA,
...
test_create.Encounter.record <- function()
{
    ...
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("type", "kps", "ecog", "date","fev1.ratio","fev1.percent","carbon.monoxide.diffusion","systolic", "diastolic","height","weight","BSA","BMI","ZubrodScore"))
    checkEquals(x[[1]], list(PatientID="TCGA.18.3406", PtNum=1, study=study, Name="Encounter", Fields=list(type=NA, kps=0, ecog=NA,
    date=NA,fev1.ratio=NA,fev1.percent=NA, carbon.monoxide.diffusion=NA, systolic=NA, diastolic=NA,height=NA,weight=NA,BSA=NA,
    BMI=NA, ZubrodScore=NA)))
...
create.Procedure.record <- function(patient.id)
{
    ...
    if(nrow(tbl.f1Sub)>0){
        for(Event in 1:nrow(tbl.f1Sub)){
            ...
            if(date != "[Not Available]" & date != "[Not Applicable]" & !is.na(diagnosis.date)){
    ...
    if(nrow(tbl.nteSub)>0){
        for(Event in 1:nrow(tbl.nteSub)){
            date <- tbl.nteSub$new_tumor_event_surgery_days_to_loco[Event]
            if(date != "[Not Available]" & !is.na(diagnosis.date)){
                date = reformatDate(diagnosis.date + as.integer(date))
                new.event <- list(PatientID=patient.id,
                PtNum=patient.number,
                study=study,
            ...
            date <- tbl.nteSub$new_tumor_event_surgery_days_to_met[Event]
            if(date != "[Not Available]" & !is.na(diagnosis.date)){
                date = reformatDate(diagnosis.date + as.integer(date))
                new.event <- list(PatientID=patient.id,
                PtNum=patient.number,
    ...
    if(nrow(tbl.omfSub)>0){
        for(Event in 1:nrow(tbl.omfSub)){
            ...
            if(date == "[Not Available]" & date == "[Pending]" & is.na(diagnosis.date)){ date = NA
...
test_create.Procedure.record <- function()
{
    ...
    x <- create.Procedure.record("TCGA-NK-A7XE")
    checkTrue(is.list(x))
    checkEquals(x[[1]], list(PatientID="TCGA.NK.A7XE", PtNum=488, study=study, Name="Procedure", Fields=list(date="06/12/2004", service=NA, name="Prostectomy", site=NA, side=NA, location=NA)))
    
    x <- create.Procedure.record("TCGA-21-5786")
    checkEquals(names(x[[1]]), c("PatientID", "PtNum","study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "service", "name", "site", "side", "location"))
     checkEquals(x[[1]], list(PatientID= "TCGA.21.5786", PtNum=34, study=study, Name="Procedure", Fields=list(date="04/19/2011", service=NA, name=NA, site="locoregional", side=NA, location=NA)))
...
create.all.Procedure.records <- function(patient.ids)
{
    ...
    tbl.omfSub <- subset(tbl.omf, bcr_patient_barcode %in% patient.ids & days_to_surgical_resection != "[Not Available]" & days_to_surgical_resection != "[Not Applicable]" & days_to_surgical_resection != "[Pending]" )
    ids <- unique(c(tbl.f1Sub$bcr_patient_barcode,tbl.omfSub$bcr_patient_barcode,tbl.nteSub$bcr_patient_barcode ) )
...
create.Pathology.record <- function(patient.id)
{
   ...
   for(pathEvent in 1:nrow(tbl.pathSub)){
      ...
      patient.id <- gsub("-", ".", patient.id, fixed=TRUE)
      patient.number <- as.integer(id.map[patient.id])
      if(tbl.pathSub$days_to_initial_pathologic_diagnosis[pathEvent] == "[Not Available]"|
      tbl.pathSub$days_to_initial_pathologic_diagnosis[pathEvent] == "[Completed]"){
          pathology.offset = NA
      }else{
          pathology.offset <-   as.integer(tbl.pathSub$days_to_initial_pathologic_diagnosis[pathEvent])
      }
      if(is.na(diagnosis.date) | is.na(pathology.offset)){
          date = NA
      }else{
      ...
      new.event <- list(PatientID=patient.id,
                        ...
                        Fields = list(date=date, disease=pathDisease, histology=pathHistology, collection=collection, T.Stage=T.Stage, N.Stage=N.Stage, M.Stage=M.Stage,
                            S.Stage=S.Stage,staging.System=staging.System,method=NA))
      good.records.found <- good.records.found + 1
      result[[good.records.found]] <- new.event
      }
   } # for pathEvent
     ...
     for(omfEvent in 1:nrow(tbl.omfSub)){
      ...
      if(histology == "[Not Available]" | histology == "[Pending]") histology = NA
      if(omfOffset == "[Not Available]" | omfOffset == "[Pending]"){ omfOffset = NA
      }else{ omfOffset = as.integer(omfOffset) }
      if(is.na(diagnosis.date) | is.na(omfOffset)){
          omf.date = NA
      }else{
       ...
       new.event <- list(PatientID=patient.id,
                        ...
                        study=study,
                        Name=name,
                        Fields = list(date=omf.date, disease=disease, histology=histology, collection=NA, T.Stage=NA, N.Stage=NA, M.Stage=NA,S.Stage=NA,staging.System=NA,method=NA))
...
test_create.Pathology.record <- function()
{
    ...
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(x[[1]], list(PatientID="TCGA.18.3406", PtNum=1, study=study, Name="Pathology",Fields=list(date= "01/01/2003", disease="Lung", histology= "Lung Squamous Cell Carcinoma", collection="retrospective",T.Stage="T1",N.Stage="N0",M.Stage="M0",S.Stage="Stage IA",staging.System=NA,method=NA)))
    checkEquals(x[[2]], list(PatientID="TCGA.18.3406", PtNum=1, study=study, Name="Pathology",Fields=list(date= NA, disease="Lung", histology="Squamous Cell Carcinoma, Not Otherwise Specified", collection=NA,T.Stage=NA,N.Stage=NA,M.Stage=NA,S.Stage=NA,staging.System=NA,method=NA)))
    x <- create.Pathology.record("TCGA-66-2769") #has omf
    checkEquals(x[[1]], list(PatientID="TCGA.66.2769", PtNum=293, study=study, Name="Pathology",Fields=list(date= "01/01/2007", disease="Lung", histology= "Lung Squamous Cell Carcinoma", collection="retrospective",T.Stage="T4",N.Stage="N0",M.Stage="M0",S.Stage="Stage IIIB",staging.System="6th",method=NA)))
} # test_create.Pathology.record
#------------------------------------------------------------------------------------------------------------------------
...
create.Absent.record <- function(patient.id)
{
   ...
   if(nrow(tbl.nteSub) == 0 && nrow(tbl.omfSub) == 0)
       return(list())
   diagnosis.year <- subset(tbl.pt, bcr_patient_barcode==patient.id)$initial_pathologic_dx_year[1]
   if( diagnosis.year == "[Not Available]"){
       diagnosis.year = NA
  ...
  if(length(duplicates)>0)
   ...
   for(Event in 1:nrow(tbl.rows)){
      ...
      if(tbl.rows$new_tumor_event_dx_days_to[Event] == "[Not Available]" |
         tbl.rows$new_tumor_event_dx_days_to[Event] == "[Pending]" ){
             dx.offset = NA
      }else{
             dx.offset = as.integer(tbl.rows$new_tumor_event_dx_days_to[Event])
      }
         
      if(is.na(dx.offset) | is.na(diagnosis.date)){
          date = NA
      }else{
...
test_create.Absent.record <- function()
{
    ...
    x <- create.Absent.record("TCGA-33-AASJ")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "Radiation", "Drug"))
    checkEquals(x[[1]], list(PatientID="TCGA.33.AASJ", PtNum=92, study=study, Name="Absent", Fields=list(date="02/09/2002", Radiation="TRUE", Drug="TRUE")))
} # test_create.Absent.record
#------------------------------------------------------------------------------------------------------------------------
...
create.Background.record <- function(patient.id)
{
    ...
    if(nrow(tbl.ptSub)>0){
            ...
            alcohol.usage.yrs=alcohol.usage.yrs, alcohol.amount=alcohol.amount, neoadjuvant.treatment=neoadjuvant.treatment))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
    }
    result[1:good.records.found]
...
{
    print("--- test_create.History.record")
    ...
    checkEquals(names(x[[1]][["Fields"]]), c("tobacco.usages","smoking.status","num.packs.years","num.packs.day",
               "alcohol.usage.yrs", "alcohol.amount","neoadjuvant.treatment"))
    checkEquals(x[[1]], list(PatientID="TCGA.34.5231", PtNum=100, study=study, Name="Background", Fields=list(tobacco.usages=c(NA,NA), smoking.status=NA, num.packs.years="50",num.packs.day=NA, alcohol.usage.yrs=NA, alcohol.amount=NA, neoadjuvant.treatment="No")))
    
    x <- create.Background.record("TCGA-18-3407")
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("tobacco.usages","smoking.status","num.packs.years","num.packs.day",
    "alcohol.usage.yrs", "alcohol.amount","neoadjuvant.treatment"))
    checkEquals(x[[1]], list(PatientID="TCGA.18.3407", PtNum=2, study=study, Name="Background", Fields=list(tobacco.usages=c(NA,"1988"), smoking.status=NA, num.packs.years="40",num.packs.day=NA, alcohol.usage.yrs=NA, alcohol.amount=NA, neoadjuvant.treatment="No")))
...
create.Tests.record <- function(patient.id)
{
    tbl.ptSub <- subset(tbl.pt, bcr_patient_barcode==patient.id & (kras_mutation_found != "[Not Available]" |
                        egfr_mutation_status != "[Not Available]" | eml4_alk_translocation_status != "[Not Available]"|
                        pulmonary_function_test_indicator != "[Not Available]"))
    ...
    for(Event in 1:nrow(tbl.ptSub)){
        if(tbl.ptSub[Event,]$kras_mutation_found != "[Not Available]"){
            ...
            else res = tbl.ptSub[Event,]$kras_mutation_identified_type
        ...
        if(tbl.ptSub[Event,]$egfr_mutation_status != "[Not Available]"){
            test="egfr"
            
            if( tbl.ptSub[Event,]$egfr_mutation_status == "NO" ) res = "Not found"
            else res = tbl.ptSub[Event,]$egfr_mutation_identified_type
            if(res == "[Not Available]") res=NA
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=NA, Type=NA,Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
        if(tbl.ptSub[Event,]$eml4_alk_translocation_status != "[Not Available]"){
            test="eml4_alk_translocation"
            
            if( tbl.ptSub[Event,]$eml4_alk_translocation_status == "NO" ) res = "Not found"
            else res = tbl.ptSub[Event,]$eml4_alk_translocation_variant
            if(res == "[Not Available]") res=NA
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=NA, Type=NA,Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
        if(tbl.ptSub[Event,]$pulmonary_function_test_indicator != "[Not Available]"){
            test="pulmonary_function"
            
            if( tbl.ptSub[Event,]$pulmonary_function_test_indicator == "NO" ) res = "Not Done"
            else res = tbl.ptSub[Event,]$pulmonary_function_test_indicator
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=NA, Type=NA,Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
...
{
    print("--- test_create.Absent.record")
    x <- create.Tests.record("TCGA-21-5783")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
