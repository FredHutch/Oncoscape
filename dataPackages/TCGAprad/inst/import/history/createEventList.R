...
run <- function()
{
   ...
   checkEquals(as.list(table(unlist(lapply(history, function(e) e["Name"])))), list(`Absent`=50, `Background`= 508,`Birth`=520, `Diagnosis`=520,`Drug`=354,`Encounter`=1530, `Pathology`=625,`Procedure`=99, `Progression`=30,  `Radiation`=104, `Status`=520, `Tests`=646))
...
test_create.DOB.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.05.4405", PtNum=15, study="TCGAprad", Name="Birth", Fields=list(date="06/03/1931", gender="female", race=NA, ethnicity=NA)))
    x <- create.DOB.record(tcga.ids[100])
    checkEquals(x, list(PatientID="TCGA.49.4486", PtNum=100, study="TCGAprad", Name="Birth", Fields=list(date="09/06/1919", gender="male", race="white", ethnicity="not hispanic or latino")))
  ...
  if(length(duplicates)>0)
   ...
   if(nrow(tbl.ptSub)>0){
    for(Event in 1:nrow(tbl.ptSub)){
        pulmonary=tbl.ptSub$pulmonary_function_test_indicator[Event]
        if(pulmonary == "NO"){ pul = "TRUE"
        } else if(pulmonary == "YES"){ pul = "FALSE"
        } else { pul = NA}
               ...
               new.event <- list(PatientID=patient.id,
                                 ...
                                 study=study,
                                 Name=name,
                                 Fields = list(date=NA, Radiation=NA, Drug=NA, pulmonary=pul))
               good.records.found <- good.records.found +1
               result[[good.records.found]] <- new.event
       }
...
test_create.Absent.record <- function()
{
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.62.A46O", PtNum=261, study=study, Name="Absent", Fields=list(date="08/01/2008", Radiation="TRUE", Drug="TRUE", pulmonary=NA)))
    
    x <- create.Absent.record("TCGA-38-4630") #has omf
    checkEquals(x[[1]], list(PatientID="TCGA.38.4630", PtNum=45, study=study, Name="Absent",Fields=list(date=NA, Radiation=NA, Drug=NA, pulmonary="TRUE")))
