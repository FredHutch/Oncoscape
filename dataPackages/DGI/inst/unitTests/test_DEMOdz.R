...
testManifest <- function()
{
   ...
   checkEquals(rownames(tbl), c("mtx.mrna.ueArray.RData","mtx.mrna.bc.RData", "mtx.mut.RData", "mtx.cn.RData",
                                "history.RData", "tbl.ptHistory.RData", "mtx.prot.RData",
                                "mtx.meth.RData", "genesets.RData", "markers.json.RData"))

   checkEquals(sort(tbl$class), c("character", "data.frame", "list", "list", "matrix", "matrix",
                                  "matrix", "matrix", "matrix", "matrix"))

   checkEquals(sort(tbl$category), c("copy number", "geneset", "history",
                                     "history", "mRNA expression", "mRNA expression", "methylation",
...
testConstructor <- function()
{
   ...
   dp <- DEMOdz();
   checkEquals(dim(manifest(dp)), c(10, 11))
   checkEquals(length(matrices(dp)), 6)
   checkEquals(sort(names(matrices(dp))), c("mtx.cn", "mtx.meth", "mtx.mrna.bc","mtx.mrna.ueArray", "mtx.mut", "mtx.prot"))
   checkEquals(eventCount(history(dp)), 201)
...
testNetworks <- function()
{
   ...
   checkEquals(length(networks), 1)
   checkEquals(names(networks), "g.markers.json")
