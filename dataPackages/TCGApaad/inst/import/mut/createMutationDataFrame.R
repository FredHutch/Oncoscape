...
library(RUnit)
directory = "../../../../RawData/"
diseaseName = "TCGApaad"

table.mut <- read.table(file=paste0(directory, diseaseName, "/mysql_cbio_mut.txt"), header=T, as.is=T)
## 71218 samples x gene protein changes

samples <- unique(table.mut[,"sample_id"])
sample.tbl <- read.delim(file=paste0(directory, diseaseName, "/mysql_cbio_samples.txt"), header=T, as.is=T, sep="\t")
...
genes.tbl <- read.delim(file=paste0(directory, diseaseName, "/mysql_cbio_genes.txt"), header=T, as.is=T, sep="\t")
...
checkEquals(mtx.mut["TCGA.IB.7651.01", "OR4K14"], "A230T;G97*")
checkEquals(mtx.mut["TCGA.IB.7651.01", "MYO15A"], "G1805C;A1183S;R1659W;V2512M;D1037G;Q1266H;D2225G;P958L;G3406D;P1289_splice;Q1070H;S3065I;R114S;R3294W;S3196G")
checkEquals(mtx.mut["TCGA.HZ.A77P.01", "DNAH10"], "")

checkEquals(dim(mtx.mut), c(91, 11188))
