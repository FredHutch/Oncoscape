...
#      choose paad, #cases with data (528 on 2/10/15)
#      choose "Biotab"
#      click "build archive"
...
#
#      biospecimen_aliquot_paad.txt
#      biospecimen_analyte_paad.txt
#      biospecimen_cqcf_paad.txt                 cqcf: case quality control form
#      biospecimen_diagnostic_slides_paad.txt
#      biospecimen_normal_control_paad.txt
#      biospecimen_portion_paad.txt
#      biospecimen_protocol_paad.txt
#      biospecimen_sample_paad.txt
#      biospecimen_shipment_portion_paad.txt
#      biospecimen_slide_paad.txt
#      biospecimen_tumor_sample_paad.txt
#      clinical_drug_paad.txt
#      clinical_follow_up_v1.0_paad.txt
#      clinical_follow_up_v1.0_nte_paad.txt
#      clinical_nte_paad.txt
#      clinical_omf_v4.0_paad.txt
#      clinical_patient_paad.txt
#      clinical_radiation_paad.txt
#
...
#   clinical_radiation_paad.txt
#   clinical_follow_up_v1.0_paad.txt
#   clinical_follow_up_v1.0_nte_paad.txt      nte: new tumor event
#   clinical_nte_paad.txt
#   clinical_omf_v4.0_paad.txt                omf: other malignancy form
#
#------------------------------------------------------------------------------------------------------------------------
...
currDir <- getwd()
setwd("../../../../RawData/TCGApaad/Clinical_3-3-16/")

study="TCGApaad"

tbl.pt <- read.table("nationwidechildrens.org_clinical_patient_paad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
tbl.drug <- read.table("nationwidechildrens.org_clinical_drug_paad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.drug <- tbl.drug[3:nrow(tbl.drug),]
tbl.rad <- read.table("nationwidechildrens.org_clinical_radiation_paad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.rad <- tbl.rad[3:nrow(tbl.rad),]
tbl.f1 <- read.table("nationwidechildrens.org_clinical_follow_up_v4.4_paad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f1 <- tbl.f1[3:nrow(tbl.f1),]
tbl.f2 <- read.table("nationwidechildrens.org_clinical_follow_up_v4.4_nte_paad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f2 <- tbl.f2[3:nrow(tbl.f2),]
tbl.nte <- read.table("nationwidechildrens.org_clinical_nte_paad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.nte <- tbl.nte[3:nrow(tbl.nte),]
tbl.omf <- read.table("nationwidechildrens.org_clinical_omf_v4.0_paad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
run <- function()
{
    ...
    checkEquals(length(history), 2044)
    checkEquals(as.list(table(unlist(lapply(history, function(e) e["Name"])))), list(`Absent`=79, `Background`= 185,`Birth`=185, `Diagnosis`=185,`Drug`=255,`Pathology`=208,`Procedure`=196, `Progression`=102,  `Radiation`=49, `Status`=185, `Tests`=415))
...
parseEvents = function(patient.ids=NA)
{
    ...
    # paad surgery at time of Dx & not reported, but other post/pre surgeries in other tables
...
create.DOB.record <- function(patient.id)
{
    ...
    if(tbl.pt.row$birth_days_to == "[Not Available]" || is.na(diagnosis.date)){ dob= NA
    ...
    return(list(PatientID=patient.id, PtNum=patient.number, study="TCGApaad", Name="Birth", Fields= list(date=c(dob), gender=gender, race=race, ethnicity=ethnicity)))
...
test_create.DOB.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.2J.AABT", PtNum=15, study="TCGApaad", Name="Birth", Fields=list(date="10/12/1939", gender="female", race="white", ethnicity= NA)))
    x <- create.DOB.record(tcga.ids[100])
    checkEquals(x, list(PatientID="TCGA.HZ.8317", PtNum=100, study="TCGApaad", Name="Birth", Fields=list(date="12/20/1942", gender="female", race="white", ethnicity="not hispanic or latino")))
...
create.Chemo.record <- function(patient.id)
{
    ...
    ## colnames tbl.drug
		...
		#[13] "days_to_stem_cell_transplantation"   "number_cycles"                      
		#[15] "pharm_regimen"                       "pharm_regimen_other"                
		#[17] "prescribed_dose"                     "prescribed_dose_units"              
		#[19] "regimen_indication"                  "regimen_indication_notes"           
		#[21] "regimen_number"                      "route_of_administration"            
		#[23] "stem_cell_transplantation"           "stem_cell_transplantation_type"     
		#[25] "therapy_type_notes"                  "total_dose"                         
		#[27] "total_dose_units"                    "tx_on_clinical_trial"               
    ...
    if(nrow(tbl.drugSub)>0){
        for(chemoEvent in 1:nrow(tbl.drugSub)){
            ...
            intent          <- tbl.drugSub$pharm_regimen[chemoEvent]
            dose            <- tbl.drugSub$prescribed_dose[chemoEvent]
            totalDose       <- tbl.drugSub$total_dose[chemoEvent]
            units           <- tbl.drugSub$prescribed_dose_units[chemoEvent]
            totalDoseUnits  <- tbl.drugSub$total_dose_units[chemoEvent]
            route           <- tbl.drugSub$route_of_administration[chemoEvent]
            cycle           <- tbl.drugSub$number_cycles[chemoEvent]
            #    $treatment   neoadjuvant, adjuvant
            #    $disease     "Brain"
            #    $schedule    5/28, every 2 weeks
            ...
            if(intent == "OTHER: SPECIFY IN NOTES") intent <- tbl.drugSub$pharm_regimen_other[chemoEvent]
...
test_create.Chemo.record <- function()
{
    ...
    x <- create.Chemo.record(tcga.ids[13])
    checkTrue(is.list(x))
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.2J.AABP", PtNum=13, study=study, Name="Drug", Fields=list(date=c("02/24/2013", "08/26/2013"), therapyType="Chemotherapy", agent= "Gemcitabine", intent=NA, dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
...
test_create.Diagnosis.record <- function()
{
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.2J.AAB1", PtNum=1, study=study, Name="Diagnosis", Fields=list(date="01/01/2012", disease="Pancreas", siteCode="2J")))
...
test_create.status.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.2J.AAB1", PtNum=1, study=study, Name="Status", Fields=list(date="03/07/2012", status="Dead", tumorStatus="tumor free")))
    x <- create.status.record(tcga.ids[73])
    checkEquals(x, list(PatientID="TCGA.FZ.5924", PtNum=73, study=study, Name="Status", Fields=list(date="04/25/2009", status="Dead", tumorStatus="with tumor")))
} # test_create.status.record
#------------------------------------------------------------------------------------------------------------------------
...
create.Progression.record <- function(patient.id)
{
    ...
    tbl.pt.rows <- subset(tbl.pt, bcr_patient_barcode==patient.id)
    diagnosis.year <- tbl.pt.rows[,"initial_pathologic_dx_year"]
    if(diagnosis.year == "[Not Available]"){
        diagnosis.date = NA
    }else{
        diagnosis.date <- as.Date(sprintf("%s-%s-%s", diagnosis.year, "01", "01"))
   ...
   tbl.fu.rows <- tbl.fu.rows[, c("new_tumor_event_type","new_tumor_event_dx_days_to")  ]
    ...
    if(nrow(tbl.pt.rows)>0) {
	    pt <- tbl.pt.rows[, c("new_tumor_event_dx_indicator","days_to_tumor_progression")  ]
   		colnames(pt) <- c("new_tumor_event_type","new_tumor_event_dx_days_to")
   		tbl.fu.rows <- rbind(tbl.fu.rows, pt)
   	}
    if(nrow(tbl.nte.rows)>0) {
    ...
    if(nrow(tbl.fu.rows)>0){
        ...
        for(i in 1:nrow(tbl.fu.rows)){
            ...
            if(row["new_tumor_event_dx_days_to"] != "[Not Available]" & row["new_tumor_event_dx_days_to"] != "[Not Applicable]"){
                ...
                progressionVal = NA
                if(!is.na(progression)){ progressionNumber = progressionNumber+1 ; progressionVal = progressionNumber}
                
                new.event <- list(PatientID=patient.id,
                PtNum=patient.number,
                study=study,
                Name=name,
                Fields=list(date=progression, event=eventtype, number=progressionVal))
...
test_create.Progression.record <- function()
{
    ...
    x <- create.Progression.record(tcga.ids[180])
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "event", "number"))
    checkEquals(x[[1]], list(PatientID="TCGA.XN.A8T3", PtNum=180, study=study, Name="Progression", Fields=list(date="04/06/2013", event="Distant Metastasis", number=1)))
 #   checkEquals(x[[2]], list(PatientID="TCGA.XN.A8T3", PtNum=180, study=study, Name="Progression", Fields=list(date=NA, event="Distant Metastasis", number=NA)))
    x <- create.Progression.record(tcga.ids[123])
    checkEquals(length(x), 0)
    x <- create.Progression.record(tcga.ids[154])
    checkEquals(x[[1]], list(PatientID="TCGA.IB.AAUW", PtNum=154, study=study, Name="Progression", Fields=list(date="07/05/2013", event="Locoregional Recurrence", number=1)))

} # test_create.Progression.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Progression.records <- function(patient.ids)
{
    tbl.sub <- subset(tbl.pt, bcr_patient_barcode %in% patient.ids & days_to_tumor_progression != "[Not Available]" & days_to_tumor_progression != "[Not Applicable]"  )
    tbl.fu <- subset(tbl.f2, bcr_patient_barcode %in% patient.ids & new_tumor_event_dx_days_to != "[Not Available]" & new_tumor_event_dx_days_to != "[Not Applicable]"  )
    ...
    ids <- unique(c(tbl.sub$bcr_patient_barcode,tbl.fu$bcr_patient_barcode, tbl.nteSub$bcr_patient_barcode))
...
create.Radiation.record <- function(patient.id)
{
    ...
    tbl.radSub <- subset(tbl.rad, bcr_patient_barcode==patient.id )
    ...
    if(nrow(tbl.radSub)>0){
        for(radEvent in 1:nrow(tbl.radSub)){
            ...
            if(radType == "[Not Available]" | radType == "[Unknown]"){radType=NA
            }else if(radType == "OTHER: SPECIFY IN NOTES"){
                radType = tbl.radSub$radiation_type_other[radEvent]
                if(radType == "[Not Applicable]"){ radType = NA
                }
            }else if( grepl("Stereotactic Radiosurgery", radType, ignore.case=TRUE) |
            ...
            intent          <- tbl.radSub$regimen_indication[radEvent]
            ...
            if(intent == "OTHER: SPECIFY IN NOTES") intent = tbl.radSub$regiment_indication_notes[radEvent]
...
test_create.Radiation.record <- function()
{
    ...
    x <- create.Radiation.record(tcga.ids[166])
    checkEquals(length(x), 0)

    x <- create.Radiation.record("TCGA-2J-AAB9")    
    checkEquals(x[[1]], list(PatientID="TCGA.2J.AAB9", PtNum=5, study=study, Name="Radiation", Fields=list(date=c(NA,NA), therapyType="external", intent=NA, target="Primary Tumor Field", totalDose=NA, totalDoseUnits=NA, numFractions=NA)))
    x <- create.Radiation.record("TCGA-2J-AABE")    
    checkEquals(x[[1]], list(PatientID="TCGA.2J.AABE", PtNum=7, study=study, Name="Radiation", Fields=list(date=c("07/25/2012","08/31/2012"), therapyType="external", intent=NA, target="Primary Tumor Field", totalDose=5040, totalDoseUnits="cGy", numFractions=28)))

} # test_create.Radiation.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Radiation.records <- function(patient.ids)
{
    tbl.good <- subset(tbl.rad, bcr_patient_barcode %in% patient.ids )
...
test_create.Encounter.record <- function()
{
    ...
    colNames <- c("kps","ecog","systolic","diastolic","height","weight","bsa","bmi","zubrod")
    ids <- c()
    if(any(colNames %in% tolower(colnames(tbl.ptSub)))==TRUE){
        ids <- c(ids,tbl.ptSub$bcr_patient_barcode)
    }
    if(any(colNames %in% tolower(colnames(tbl.fuSub)))==TRUE){
...
create.Procedure.record <- function(patient.id)
{
    ...
    tbl.ptSub  <- subset(tbl.pt,  bcr_patient_barcode==patient.id & surgical_procedure != "[Not Available]")
    ...
    #tbl.nteSub <- subset(tbl.nte,  bcr_patient_barcode==patient.id &
    #(new_tumor_event_surgery_days_to_loco != "[Not Available]" | new_tumor_event_surgery_days_to_met != "[Not Available]" ) &
    #(new_tumor_event_surgery_days_to_loco != "[Not Applicable]" | new_tumor_event_surgery_days_to_met != "[Not Applicable]"))
    tbl.omfSub <- subset(tbl.omf, bcr_patient_barcode==patient.id & (days_to_surgical_resection != "[Not Available]") & (days_to_surgical_resection != "[Not Applicable]"))
    
    #if( nrow(tbl.f1Sub) ==0 && nrow(tbl.nteSub) ==0 && nrow(tbl.omfSub) ==0)
    if(nrow(tbl.omfSub)==0 && tbl.ptSub ==0)
    return(list())
    ...
    result <- vector("list",  nrow(tbl.omfSub))
    good.records.found <- 0
    if(nrow(tbl.ptSub)>0){
    for(Event in 1:nrow(tbl.ptSub)){
         Surgeryname <- tbl.ptSub$surgical_procedure[Event]
        if(Surgeryname == "[Not Available]") Surgeryname = NA
        site <- tbl.ptSub$anatomic_neoplasm_subdivision[Event]
        if(site == "Other (please specify)") site = tbl.ptSub$anatomic_neoplasm_subdivision_other[Event]
        if(site == "[Not Available]" | site == "[Unknown]" | site == "[Not Applicable]") site = NA
        
        new.event <- list(PatientID=patient.id,
        PtNum=patient.number,
        study=study,
        Name=name,
        Fields = list(date=NA, service=NA, name = Surgeryname, site = site, side = NA, location = NA))
        good.records.found <- good.records.found + 1
        result[[good.records.found]] <- new.event
    } }
	if(nrow(tbl.omfSub)>0){
    for(Event in 1:nrow(tbl.omfSub)){
        ...
        if(date == "[Not Available]" | date == "[Not Applicable]" | date == "[Pending]"){ date = NA
        ...
        site <- tbl.omfSub$other_malignancy_anatomic_site[Event] 
        if(site == "Other") site <- tbl.omfSub$other_malignancy_anatomic_site_text[Event]
        if(side == "[Not Available]" | side == "[Unknown]" | side == "[Not Applicable]") side = NA
        if(site == "[Not Available]" | site == "[Unknown]" | site == "[Not Applicable]") site = NA
        ...
        study=study,
        Name=name,
        Fields = list(date=date, service=NA, name = Surgeryname, site = site, side = side, location = NA))
        good.records.found <- good.records.found + 1
        result[[good.records.found]] <- new.event
    } }
...
test_create.Procedure.record <- function()
{
    ...
    x <- create.Procedure.record(tcga.ids[11])
    checkEquals(length(x), 1)
    checkEquals(names(x[[1]]), c("PatientID", "PtNum","study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "service", "name", "site", "side", "location"))
    checkEquals(x[[1]], list(PatientID= "TCGA.2J.AABK", PtNum=11, study=study, Name="Procedure", Fields=list(date= NA, service=NA, name="Whipple", site="Uncinate process", side=NA, location=NA)))
    x <- create.Procedure.record("TCGA-2J-AABR")
    checkEquals(x[[1]], list(PatientID= "TCGA.2J.AABR", PtNum=14, study=study, Name="Procedure", Fields=list(date= NA, service=NA, name="Whipple", site="Head & body of pancreas", side=NA, location=NA)))
    checkEquals(x[[2]], list(PatientID= "TCGA.2J.AABR", PtNum=14, study=study, Name="Procedure", Fields=list(date= "12/04/2012", service=NA, name="Electrodessication/curretage", site="Skin", side="Left", location=NA)))
} # test_create.Procedure.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Procedure.records <- function(patient.ids)
{
    
    tbl.ptSub <- subset(tbl.pt,   bcr_patient_barcode %in% patient.ids & surgical_procedure != "[Not Available]" )
    ...
    #tbl.nteSub <- subset(tbl.nte, bcr_patient_barcode %in% patient.ids & (new_tumor_event_surgery_days_to_loco != "[Not Available]"  | new_tumor_event_surgery_days_to_met != "[Not Available]") & (new_tumor_event_surgery_days_to_loco != "[Not Applicable]" | new_tumor_event_surgery_days_to_met != "[Not Applicable]"))
    
    tbl.omfSub <- subset(tbl.omf, bcr_patient_barcode %in% patient.ids & days_to_surgical_resection != "[Not Available]" & days_to_surgical_resection != "[Not Applicable]" )
    ids <- unique(c(tbl.ptSub$bcr_patient_barcode, tbl.omfSub$bcr_patient_barcode))
...
create.Pathology.record <- function(patient.id)
{
    ...
    if(nrow(tbl.pathSub) >0){
        for(pathEvent in 1:nrow(tbl.pathSub)){
            ...
            if(pathology.offset != "[Not Available]"){
                pathology.offset <- as.integer(pathology.offset)
            ...
            if (T.Stage == "[Not Available]" | T.Stage == "[Not Applicable]" | T.Stage == "[Unknown]") T.Stage = NA
            if (N.Stage == "[Not Available]" | N.Stage == "[Not Applicable]" | N.Stage == "[Unknown]") N.Stage = NA
            if (M.Stage == "[Not Available]" | M.Stage == "[Not Applicable]" | M.Stage == "[Unknown]") M.Stage = NA
            if (S.Stage == "[Not Available]" | S.Stage == "[Not Applicable]" | S.Stage == "[Unknown]") S.Stage = NA
            ...
            S.Stage=S.Stage,staging.System=staging.System, method=pathMethod))
            good.records.found <- good.records.found + 1
            result[[good.records.found]] <- new.event
        }
    } # for pathEvent
    ...
    if(nrow(tbl.omfSub)>0){
        for(omfEvent in 1:nrow(tbl.omfSub)){
            ...
            histology.category = NA #tbl.omf.histology[tbl.omf.histology$omf.histology==histology, 4]
            ...
            if(omfOffset == "[Not Available]" | omfOffset == "[Pending]"){
                omf.date = NA
            }else{
                omf.date = reformatDate(as.Date(diagnosis.date, "%m/%d/%Y") + as.integer(omfOffset))
            }
            ...
            study=study,
            Name=name,
            Fields = list(date=omf.date, disease=disease, histology=histology, histology.category=histology.category, collection=NA, T.Stage=NA, N.Stage=NA, M.Stage=NA,S.Stage=NA,staging.System=NA, method=NA))
...
test_create.Pathology.record <- function()
{
    ...
    x <- create.Pathology.record(tcga.ids[5])
    ...
    checkEquals(x[[1]], list(PatientID= "TCGA.2J.AAB9", PtNum=5, study=study, Name="Pathology", 
        Fields=list(date="01/01/2012", disease="Pancreas", histology="Pancreas", 
        histology.category=NA, collection="retrospective", T.Stage=NA,N.Stage=NA,M.Stage=NA,S.Stage=NA,
        staging.System=NA, method="Tumor resection")))
    x <- create.Pathology.record("TCGA-3A-A9I5")
        checkEquals(x[[1]], list(PatientID= "TCGA.3A.A9I5", PtNum=24, study=study, Name="Pathology", 
        Fields=list(date="01/01/2009", disease="Pancreas", histology="Pancreas", histology.category=NA, 
        collection="retrospective", T.Stage=NA,N.Stage=NA,M.Stage=NA,S.Stage=NA,staging.System=NA, method="Tissue Biopsy")))
        checkEquals(x[[2]], list(PatientID= "TCGA.3A.A9I5", PtNum=24, study=study, Name="Pathology", 
        Fields=list(date="06/28/2004", disease="Prostate", histology="Other, specify:80003 NEOPLASM MALIGNANT", histology.category=NA, 
...
create.Absent.record <- function(patient.id)
{
    ...
    tbl.f1Sub  <- subset(tbl.f2,  bcr_patient_barcode==patient.id & new_tumor_event_dx_days_to != "[Not Available]" & (new_tumor_event_pharmaceutical_tx == "NO"))
    ...
    if(nrow(tbl.f1Sub)>0){
        f1 <- tbl.f1Sub[,c("new_tumor_event_dx_days_to","new_tumor_event_pharmaceutical_tx")]
        f1 <- cbind(f1,col_rad =rep(NA,nrow(f1)))
        f1 <- f1[,c(1,3,2)]
        colnames(f1) <- c("new_tumor_event_dx_days_to", "new_tumor_event_radiation_tx", "new_tumor_event_pharmaceutical_tx")
        tbl.rows <- rbind(tbl.rows, f1)
    ...
    if(nrow(tbl.rows) >0){
        for(Event in 1:nrow(tbl.rows)){
            dx.offset <- tbl.rows$new_tumor_event_dx_days_to[Event]
            if(dx.offset == "[Not Applicable]" | dx.offset == "[Pending]" | dx.offset == "[Not Available]"){
                date <- NA
            }else{
                dx.offset <- as.integer(dx.offset)
            ...
            if(!is.na(Rx) && Rx == "NO"){ Rx = "TRUE"
            } else if (!is.na(Rx) && Rx == "YES") { Rx = "FALSE"
            } else { Rx = NA }
...
test_create.Absent.record <- function()
{
    ...
    x <- create.Absent.record("TCGA-2J-AAB6")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "Radiation", "Drug"))
    checkEquals(x[[1]], list(PatientID="TCGA.2J.AAB6", PtNum=3, study=study, Name="Absent", Fields=list(date="06/18/2012", Radiation="TRUE", Drug=NA)))
    
    x <- create.Absent.record("TCGA-US-A77E") 
    checkEquals(x[[1]], list(PatientID="TCGA.US.A77E", PtNum=173, study=study, Name="Absent",Fields=list(date="06/15/2012", Radiation="TRUE", Drug="TRUE")))
...
create.all.Absent.records <- function(patient.ids)
{
    ...
    tbl.f1Sub  <- subset(tbl.f2,  bcr_patient_barcode %in% patient.ids & new_tumor_event_dx_days_to != "[Not Available]" & (new_tumor_event_pharmaceutical_tx == "NO"))
...
create.Background.record <- function(patient.id)
{
    ...
    his_om           <- tbl.ptSub$history_other_malignancy
    First_Tx_Outcome <- tbl.ptSub$treatment_outcome_first_course
    neoadjuvant.treatment  <- tbl.ptSub$history_neoadjuvant_treatment
    
    if(First_Tx_Outcome == "[Not Applicable]" | First_Tx_Outcome == "[Discrepancy]" | First_Tx_Outcome == "[Not Available]" | First_Tx_Outcome == "[Unknown]") First_Tx_Outcome =NA;
    ...
    return(list(PatientID=patient.id, PtNum=patient.number, study=study, Name=name, 
        ...
        num.children=NA, carcinogen.exposure=NA, occupation=NA,marital.status=NA)))
} # create.Background.record
#-----------------------------------------------------------------------------------------------------------------------
test_create.Background.record <- function()
{
    print("--- test_create.Background.record")
    x <- create.Background.record(tcga.ids[100])
    checkTrue(is.list(x))
    checkEquals(names(x), c("PatientID", "PtNum", "study", "Name", "Fields"))
    ...
    "alcohol.amount","num.children","carcinogen.exposure","occupation",
    "marital.status"))
    checkEquals(x, list(PatientID="TCGA.HZ.8317", PtNum=100, study="TCGApaad", Name="Background", 
        Fields=list(History=list(YES=c("history of other malignancy"),NO=NA),neoadjuvant.treatment="No", 
        First_Tx_Outcome=NA,tobacco.usages=NA, smoking.status=NA, num.packs.years=NA,num.packs.day=NA, 
        alcohol.usage.yrs=NA, alcohol.amount=NA, num.children=NA,carcinogen.exposure=NA,occupation=NA,marital.status=NA)))
...
{
    tbl.ptSub <- subset(tbl.pt, bcr_patient_barcode==patient.id & ( lymph_nodes_examined == "YES" ))
    ...
    for(Event in 1:nrow(tbl.ptSub)){
        if(tbl.ptSub[Event,]$lymph_nodes_examined_count != "[Not Available]"){
            test = "lymph_nodes_count"
            test.Offset <- 0
            test.date <- reformatDate(format(diagnosis.date + test.Offset))
            
            res <- tbl.ptSub$lymph_nodes_examined_count
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=test.date, Type="count", Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
        if(tbl.ptSub[Event,]$lymph_nodes_examined_he_count != "[Not Available]"){
            test = "lymph_nodes_he_count"
            test.Offset <- 0
            test.date <- reformatDate(format(diagnosis.date + test.Offset))
            
            res <- tbl.ptSub$lymph_nodes_examined_he_count
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=test.date, Type="count", Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
        if(tbl.ptSub[Event,]$lymph_nodes_examined_ihc_count != "[Not Available]"){
            test = "lymph_nodes_ihc_count"
            test.Offset <- 0
            test.date <- reformatDate(format(diagnosis.date + test.Offset))
            
            res <- tbl.ptSub$lymph_nodes_examined_ihc_count
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=test.date, Type="count", Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
...
test_create.Tests.record <- function()
{
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.2J.AAB1", PtNum=1, study=study, Name="Tests", Fields=list(Date="01/01/2012",Type="count",Test="lymph_nodes_count", Result="22")))
    checkEquals(x[[2]], list(PatientID="TCGA.2J.AAB1", PtNum=1, study=study, Name="Tests", Fields=list(Date="01/01/2012",Type="count",Test="lymph_nodes_he_count", Result="7")))
    checkEquals(x[[3]], list(PatientID="TCGA.2J.AAB1", PtNum=1, study=study, Name="Tests", Fields=list(Date="01/01/2012",Type="count",Test="lymph_nodes_ihc_count", Result="0")))
} # test_create.Tests.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Tests.records <- function(patient.ids)
{
    tbl.ptSub <- subset(tbl.pt, bcr_patient_barcode %in% patient.ids &  (lymph_nodes_examined == "YES") )
