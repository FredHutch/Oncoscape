...
#      choose coad, #cases with data (528 on 2/10/15)
#      choose "Biotab"
#      click "build archive" 
...
#
#      biospecimen_aliquot_coad.txt
#      biospecimen_analyte_coad.txt
#      biospecimen_cqcf_coad.txt                 cqcf: case quality control form
#      biospecimen_diagnostic_slides_coad.txt
#      biospecimen_normal_control_coad.txt
#      biospecimen_portion_coad.txt
#      biospecimen_protocol_coad.txt
#      biospecimen_sample_coad.txt
#      biospecimen_shipment_portion_coad.txt
#      biospecimen_slide_coad.txt
#      biospecimen_tumor_sample_coad.txt
#      clinical_drug_coad.txt
#      clinical_follow_up_v1.0_coad.txt
#      clinical_follow_up_v1.0_nte_coad.txt
#      clinical_nte_coad.txt
#      clinical_omf_v4.0_coad.txt
#      clinical_patient_coad.txt
#      clinical_radiation_coad.txt
#
...
#   clinical_radiation_coad.txt
#   clinical_follow_up_v1.0_coad.txt
#   clinical_follow_up_v1.0_nte_coad.txt      nte: new tumor event
#   clinical_nte_coad.txt
#   clinical_omf_v4.0_coad.txt                omf: other malignancy form
#
#------------------------------------------------------------------------------------------------------------------------
...
currDir <- getwd()
setwd("../../../../RawData/TCGAcoad/Clinical_05-21-15/")

study="TCGAcoad"

tbl.pt <- read.table("clinical_patient_coad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
tbl.drug <- read.table("clinical_drug_coad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.drug <- tbl.drug[3:nrow(tbl.drug),]
tbl.rad <- read.table("clinical_radiation_coad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.rad <- tbl.rad[3:nrow(tbl.rad),]
tbl.f1 <- read.table("clinical_follow_up_v1.0_coad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f1 <- tbl.f1[3:nrow(tbl.f1),]
tbl.f2 <- read.table("clinical_follow_up_v1.0_nte_coad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f2 <- tbl.f2[3:nrow(tbl.f2),]
tbl.nte <- read.table("clinical_nte_coad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
run <- function()
{
   ...
   checkTrue(all(patients %in% tbl.pt$bcr_patient_barcode))
   ...
   checkEquals(length(history), 4467)
   checkEquals(as.list(table(unlist(lapply(history, function(e) e["Name"])))), list(`Absent`=67,`Background`=454, `Birth`=454, `Diagnosis`=454,`Drug`=550,`Encounter`=454, `Pathology`=529, `Procedure`=76, `Progression`=96,  `Radiation`=31, `Status`=454, `Tests`=848))
...
create.DOB.record <- function(patient.id)
{
   ...
   return(list(PatientID=patient.id, PtNum=patient.number, study="TCGAcoad", Name="Birth", Fields= list(date=c(dob), gender=gender, race=race, ethnicity=ethnicity)))
...
test_create.Chemo.record <- function()
{
    ...
    x <- create.Chemo.record("TCGA-A6-2671")
    checkTrue(is.list(x))
    checkEquals(length(x), 22)
    checkEquals(names(x[[15]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(x[[15]], list(PatientID="TCGA.A6.2671", PtNum=5, study=study, Name="Drug", Fields=list(date=c( "07/06/2010","01/10/2011"), therapyType="Targeted Molecular therapy", agent="Avastin", intent="progression" , dose="300-325", units="mg"  , totalDose=3775, totalDoseUnits="mg", route="Intravenous (IV)", cycle=12)))
   
   x <- create.Chemo.record("TCGA-A6-A565")  #no start date
   checkEquals(length(x), 3)
   checkEquals(x[[1]], list(PatientID="TCGA.A6.A565", PtNum=51, study=study, Name="Drug", Fields=list(date=c(NA, NA), therapyType="Chemotherapy", agent="5FU", intent=NA , dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
   
   x <- create.Chemo.record("TCGA-AA-3516")  # omf chemo
   checkEquals(length(x), 1)
   checkEquals(x[[1]], list(PatientID="TCGA.AA.3516", PtNum=68, study=study, Name="Drug", Fields=list(date=c(NA, NA), therapyType=NA, agent=NA, intent="Prior Malignancy", dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
...
test_create.Diagnosis.record <- function()
{
   ...
   x <- create.Diagnosis.record(tcga.ids[1])
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.3L.AA1B", PtNum=1, study=study, Name="Diagnosis", Fields=list(date="01/01/2013", disease="Colon", siteCode="3L")))
	   ...
	   status.offset <-   as.integer(tbl.pt.row$death_days_to)
		...
		if(nrow(tbl.fu.rows) != 0 ){
		  for(i in 1:nrow(tbl.fu.rows)){		
			...
			} else{ if(is.na(status.offset) ||(row["last_contact_days_to"] != "[Not Available]" && row["last_contact_days_to"] != "[Discrepancy]"  && row["last_contact_days_to"] > status.offset)) {
...
test_create.status.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.3L.AA1B", PtNum=1, study=study, Name="Status", Fields=list(date="12/16/2013", status="Alive", tumorStatus=NA)))
    x <- create.status.record("TCGA-CK-6746")
    checkEquals(x, list(PatientID="TCGA.CK.6746", PtNum=298, study=study, Name="Status", Fields=list(date="12/30/2008", status="Alive", tumorStatus="tumor free")))
} # test_create.status.record
#------------------------------------------------------------------------------------------------------------------------
...
create.Progression.record <- function(patient.id)
{
    ...
    good.records.found <- 0
    if(all(tbl.fu.rows$new_tumor_event_dx_days_to != "[Not Available]"))
...
test_create.Progression.record <- function()
{
    ...
    x <- create.Progression.record("TCGA-NH-A6GA")  #in nte table
    checkEquals(length(x), 2)
    checkEquals(x[[1]], list(PatientID="TCGA.NH.A6GA", PtNum=442, study=study, Name="Progression", Fields=list(date="01/07/2013", event=NA, number=1)))
    checkEquals(x[[2]], list(PatientID="TCGA.NH.A6GA", PtNum=442, study=study, Name="Progression", Fields=list(date="04/17/2013", event=NA, number=2)))
    
    
    x <- create.Progression.record("TCGA-A6-2674")  #2 progression events
    checkEquals(length(x), 2)
    checkEquals(x[[1]], list(PatientID="TCGA.A6.2674", PtNum=7, study=study, Name="Progression", Fields=list(date="01/06/2011", event=NA, number=1)))
    checkEquals(x[[2]], list(PatientID="TCGA.A6.2674", PtNum=7, study=study, Name="Progression", Fields=list(date="05/04/2012", event=NA, number=2)))
    
    x <- create.Progression.record("TCGA-CM-4747")
...
test_create.Radiation.record <- function()
{
    ...
    x <- create.Radiation.record("TCGA-AA-3713")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(x[[1]], list(PatientID="TCGA.AA.3713", PtNum=123, study=study, Name="Radiation", Fields=list(date=c("10/03/2005","11/03/2005"), therapyType="external", intent=NA, target="Primary Tumor Field", totalDose=5000, totalDoseUnits="cGy", numFractions=NA)))

    x <- create.Radiation.record("TCGA-AD-6901")  #no units
    checkEquals(x[[1]], list(PatientID="TCGA.AD.6901", PtNum=237, study=study, Name="Radiation", Fields=list(date=c("05/03/2012", "05/18/2012"), therapyType="external", intent=NA, target= "Distant Recurrence", totalDose=NA, totalDoseUnits=NA, numFractions=NA)))
   
    x <- create.Radiation.record("TCGA-DM-A285") #tbl.omf
    checkEquals(x[[1]], list(PatientID="TCGA.DM.A285", PtNum=385, study=study, Name="Radiation", Fields=list(date=NA, therapyType="Prior Malignancy", intent=NA, target="Locoregional, at primary tumor site: no", totalDose=NA, totalDoseUnits=NA, numFractions=NA)))
...
test_create.Encounter.record <- function()
{
    ...
    x <- create.Encounter.record(tcga.ids[171])
    checkEquals(x[[1]], list(PatientID="TCGA.AA.3970", PtNum=171, study=study, Name="Encounter", Fields=list(type=NA, kps=NA, ecog=NA,date= "01/01/2009", height=NA,weight=NA,BSA=NA, BMI=NA, ZubrodScore=NA)))
...
test_create.Procedure.record <- function()
{
    ...
    x <- create.Procedure.record("TCGA-AD-6895") #from omf table "[Pending]"
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "service", "name", "site", "side"))
    checkEquals(x[[1]], list(PatientID="TCGA.AD.6895", PtNum=235, study=study, Name="Procedure", Fields=list(date="01/15/2011", service=NA, name=NA, site=NA, side=NA)))
    x <- create.Procedure.record("TCGA-A6-A567") #from both proc and nte tables
    checkEquals(length(x),2)
    checkEquals(x[[1]], list(PatientID="TCGA.A6.A567", PtNum=53, study=study, Name="Procedure", Fields=list(date="02/11/2010", service=NA, name=NA, site="Metastatic", side=NA)))

    checkEquals(x[[2]], list(PatientID="TCGA.A6.A567", PtNum=53, study=study, Name="Procedure", Fields=list(date="11/17/2008", service=NA, name=NA, site="Metastatic", side=NA)))
...
test_create.Absent.record <- function()
{
    ...
    x <- create.Absent.record("TCGA-A6-A565")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "Radiation", "Drug"))
    checkEquals(x[[1]], list(PatientID="TCGA.A6.A565", PtNum=51, study=study, Name="Absent", Fields=list(date="10/28/2008", Radiation="TRUE", Drug="FALSE")))
    checkEquals(x[[2]], list(PatientID="TCGA.A6.A565", PtNum=51, study=study, Name="Absent", Fields=list(date="03/15/2009", Radiation="TRUE", Drug="FALSE")))
    
    x <- create.Absent.record("TCGA-AD-6888") #has omf
    checkEquals(x[[1]], list(PatientID="TCGA.AD.6888", PtNum=232, study=study, Name="Absent",Fields=list(date=NA, Radiation="TRUE", Drug="TRUE")))

    x <- create.Absent.record("TCGA-A6-2674") #has f2: no radiation
    checkEquals(x[[1]], list(PatientID="TCGA.A6.2674", PtNum=7, study=study, Name="Absent",Fields=list(date="01/06/2011", Radiation="TRUE", Drug="FALSE")))
    checkEquals(x[[2]], list(PatientID="TCGA.A6.2674", PtNum=7, study=study, Name="Absent",Fields=list(date="05/04/2012", Radiation="TRUE", Drug="TRUE")))
...
test_create.Tests.record <- function()
{
    ...
    x <- create.Tests.record("TCGA-DM-A28E") #has two mutations
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("Date","Type","Test", "Result"))
    checkEquals(x[[1]], list(PatientID="TCGA.DM.A28E", PtNum=389, study=study, Name="Tests", Fields=list(Date=NA, Type=NA,Test="kras", Result="12")))
    checkEquals(x[[2]], list(PatientID="TCGA.DM.A28E", PtNum=389, study=study, Name="Tests", Fields=list(Date=NA, Type=NA,Test="braf", Result="Normal")))
    
    x <- create.Tests.record("TCGA-NH-A8F8")
    checkEquals(x[[1]], list(PatientID="TCGA.NH.A8F8", PtNum=446, study=study, Name="Tests",Fields=list(Date=NA, Type=NA,Test="kras", Result="12")))
    checkEquals(x[[2]], list(PatientID="TCGA.NH.A8F8", PtNum=446, study=study, Name="Tests",Fields=list(Date=NA, Type=NA,Test="braf", Result=NA)))
    checkEquals(x[[3]], list(PatientID="TCGA.NH.A8F8", PtNum=446, study=study, Name="Tests",Fields=list(Date=NA, Type=NA,Test="cea", Result="24.9")))
    x <- create.Tests.record("TCGA-A6-5662")
    checkEquals(x[[1]], list(PatientID="TCGA.A6.5662", PtNum=31, study=study, Name="Tests",Fields=list(Date=NA, Type=NA,Test="kras", Result="12")))
    checkEquals(x[[2]], list(PatientID="TCGA.A6.5662", PtNum=31, study=study, Name="Tests",Fields=list(Date=NA, Type=NA,Test="braf", Result=NA)))
    checkEquals(x[[3]], list(PatientID="TCGA.A6.5662", PtNum=31, study=study, Name="Tests",Fields=list(Date=NA, Type=NA,Test="loci", Result="tested: 5 ; abnormal count: 0")))
    checkEquals(x[[4]], list(PatientID="TCGA.A6.5662", PtNum=31, study=study, Name="Tests",Fields=list(Date=NA, Type="IHC",Test="mismatched protein", Result="tested: YES ; mismatch proteins loss: MLH1 Expressed|MSH2 Expressed|PMS2 Expressed|MSH6 Expressed")))
