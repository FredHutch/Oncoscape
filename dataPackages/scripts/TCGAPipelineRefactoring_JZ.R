...
if(ENCOUNTER){ 
  ...
  Encounter.unique.request <- function(study_name){   
		...
		data.Encounter <- tbl.f

		if(ncol(data.Encounter) == 1){
			result = list(unique.encType=NA, 
		                  unique.KPS=NA,
		                  unique.ECOG=NA,
		                  unique.height=NA,
		                  unique.weight=NA,
		                  unique.prefev1.ratio=NA,
		                  unique.prefev1.percent=NA,
		                  unique.postfev1.ratio=NA,
		                  unique.postfev1.percent=NA,
		                  unique.carbon.monoxide.diffusion=NA)
		}else{
	    # reorganize two tbls
...
if(PATHOLOGY){
  Pathology.unique.request <- function(study_name){
		...
		if(exists("tbl.omf")){
			...
			data.Pathology <- tbl.pt[,-match("dxyear", colnames(tbl.pt))]
		}
	...
	#recentDatetbl <- aggregate(date ~ PatientID, data.Status, function(x){max(x)})
 	...
 	for(i in 1:nrow(tbl.pt)){
 		...
 		if(nrow(tmpDF[which(tmpDF$date == tmpDF[1,]$date), ]) > 1){
 			...
 			if("tumorStatus" %in% colnames(tmpDF)){
...
create.Encounter.records <- function(study_name,  ptID){
		...
		if(ncol(data.Encounter) == 1){
			...
			if(all(all(is.na(data.Encounter[,c(2:11)])))) return(print(c(study_name, ncol(data.Encounter), "Result is empty")))
			else{
				# result
				ptNumMap <- ptNumMapUpdate(tbl.pt)
				if(missing(ptID)){
...
create.Procedure.records <- function(study_name,  ptID){
	...
	if(exists("tbl.f1") &&ncol(tbl.f1) > 1){
 	...
 	#if(exists("tbl.f2"))  data.Procedure <- rbind.fill(data.Procedure, tbl.f2)
	if(exists("tbl.nte_f1")) data.Procedure <- rbind.fill(data.Procedure, tbl.nte_f1) 
	if(nrow(data.Procedure) == 0){
		return("Procedure data is empty.")
	}else{
		data.Procedure <- merge(data.Procedure, tbl.pt[, c("PatientID", "dxyear")]) 
...
save(sarc, file="../TCGAsarc/history.RData")
