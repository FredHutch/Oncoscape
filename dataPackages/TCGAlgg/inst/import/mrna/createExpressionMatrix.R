...
table.mrna <- read.table(file="../../../../RawData/TCGAlgg/mysql_cbio_mrna.csv", header=F, skip=3, as.is=T)
## 527 samples(patients) x 20444 genes

sampleString <- readLines(con="../../../../RawData/TCGAlgg/mysql_cbio_mrna.csv", n=2)[[2]]
...
checkEquals(dim(mtx.mrna), c(527, 20444))
  # all good values
checkEquals(length(which(is.na(mtx.mrna))), 761515)
