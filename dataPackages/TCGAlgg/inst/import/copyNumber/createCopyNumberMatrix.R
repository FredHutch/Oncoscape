...
table.cn <- read.table(file="../../../../RawData/TCGAlgg/mysql_cbio_cna.csv", header=F, skip=3, as.is=T)
## 513 samples(patients) x 22184 genes

sampleString <- readLines(con="../../../../RawData/TCGAlgg/mysql_cbio_cna.csv", n=2)[[2]]
...
mtx.cn <- matrix(as.integer(unlist(list.cn)), nrow = length(samples), byrow = F)
dimnames(mtx.cn) <- list(BarcodeSample,HugoGenes)

checkEquals(as.list(table(mtx.cn)), list(`-2`=66318,`-1`=1333729,  `0`=9273166, `1`=631033, `2`=76146))
