...
#      choose GBM, #cases with data (527 today)
#      choose "Biotab"
#      click "build archive" 
...
test_create.Procedure.record <- function()
{
    ...
    x <- create.Procedure.record("TCGA-02-0114")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "service", "name", "site", "side"))
    checkEquals(x[[1]], list(PatientID="TCGA.02.0114", PtNum=188, study=study, Name="Procedure", Fields=list(date=NA, service=NA, name=NA, site="Locoregional Disease", side=NA)))
    x <- create.Procedure.record("TCGA-06-0125") #pre-operative timing
    checkEquals(x[[1]], list(PatientID="TCGA.06.0125", PtNum=287, study=study, Name="Procedure", Fields=list(date="03/24/2003", service=NA, name=NA, site="Locoregional Disease", side=NA)))
...
create.Pathology.record <- function(patient.id)
{
   ...
   for(pathEvent in 1:nrow(tbl.pathSub)){
      ...
      new.event <- list(PatientID=patient.id,
                        ...
                        study=study,
                        Name=name,
                        Fields = list(date=date, disease=pathDisease, histology=pathHistology, collection=collection, grade="G4"))
      good.records.found <- good.records.found + 1
      result[[good.records.found]] <- new.event
      }} # for pathEvent
     ...
     for(omfEvent in 1:nrow(tbl.omfSub)){
      ...
      if(omfOffset == "[Not Available]"){ omf.date = NA
       ...
       new.event <- list(PatientID=patient.id,
                        ...
                        study=study,
                        Name=name,
                        Fields = list(date=omf.date, disease=disease, histology=histology, collection=NA, grade="G4"))
...
test_create.Pathology.record <- function()
{
    ...
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "disease", "histology", "collection", "grade"))
    checkEquals(x[[1]], list(PatientID="TCGA.02.0001", PtNum=1, study=study, Name="Pathology", Fields=list(date="01/01/2002", disease="Brain", histology="Untreated primary (de novo) GBM", collection=NA, grade="G4")))
    ...
    checkEquals(x[[2]], list(PatientID="TCGA.06.0209", PtNum=372, study=study, Name="Pathology",Fields=list(date=NA, disease="Prostate", histology="Adenocarcinoma, Not Otherwise Specified", collection=NA, grade="G4")))
