...
var fileNames = fs.readdirSync(dataDir).filter(function(d){ return d.indexOf("brca")!=-1; }).filter(function(d){ return d.indexOf("metadata")==-1; });
	...
	for (i=0; i<fileNames.length; i++){
    	...
    	data = data.map(function(item){
    		if (item.patient_ID!=undefined){
    			item.patient_ID = item.patient_ID.replace(/\./g,"-");
    		}
    		return item;
    	});
