...
#      choose brca, #cases with data (528 on 2/10/15)
#      choose "Biotab"
#      click "build archive" 
...
#
#      biospecimen_aliquot_brca.txt
#      biospecimen_analyte_brca.txt
#      biospecimen_cqcf_brca.txt                 cqcf: case quality control form
#      biospecimen_diagnostic_slides_brca.txt
#      biospecimen_normal_control_brca.txt
#      biospecimen_portion_brca.txt
#      biospecimen_protocol_brca.txt
#      biospecimen_sample_brca.txt
#      biospecimen_shipment_portion_brca.txt
#      biospecimen_slide_brca.txt
#      biospecimen_tumor_sample_brca.txt
#      clinical_drug_brca.txt
#      clinical_follow_up_v1.0_brca.txt
#      clinical_follow_up_v1.0_nte_brca.txt
#      clinical_nte_brca.txt
#      clinical_omf_v4.0_brca.txt
#      clinical_patient_brca.txt
#      clinical_radiation_brca.txt
#
...
#   clinical_radiation_brca.txt
#   clinical_follow_up_v1.0_brca.txt
#   clinical_follow_up_v1.0_nte_brca.txt      nte: new tumor event
#   clinical_nte_brca.txt
#   clinical_omf_v4.0_brca.txt                omf: other malignancy form
#
#------------------------------------------------------------------------------------------------------------------------
...
currDir <- getwd()
setwd("../../../../RawData/TCGAbrca/Clinical_7-15-15/")

study="TCGAbrca"

tbl.pt <- read.table("clinical_patient_brca.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
tbl.drug <- read.table("clinical_drug_brca.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.drug <- tbl.drug[3:nrow(tbl.drug),]
tbl.rad <- read.table("clinical_radiation_brca.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.rad <- tbl.rad[3:nrow(tbl.rad),]
tbl.f1 <- read.table("clinical_follow_up_v2.1_brca.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f1 <- tbl.f1[3:nrow(tbl.f1),]
tbl.f2 <- read.table("clinical_follow_up_v4.0_brca.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f2 <- tbl.f2[3:nrow(tbl.f2),]

tbl.f3 <- read.table("clinical_follow_up_v4.0_nte_brca.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f3 <- tbl.f2[3:nrow(tbl.f2),]
tbl.nte <- read.table("clinical_nte_brca.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
PR_cols      <- colnames(tbl.pt)[grep("_pr_",colnames(tbl.pt))]
cent_pattern <- "_cent_17|_cent17_"
CENT17_cols  <- colnames(tbl.pt)[grep(cent_pattern,colnames(tbl.pt))]
pt_cols      <- unique(c(Her2_cols, ER_cols, PR_cols, CENT17_cols))
f.Her2_cols    <- colnames(tbl.f1)[grep("her2",colnames(tbl.f1))]
f.ER_cols      <- colnames(tbl.f1)[grep("_er_",colnames(tbl.f1))]
f.PR_cols      <- colnames(tbl.f1)[grep("_pr_",colnames(tbl.f1))]
f.cent_pattern <- "_cent_17|_cent17_"
f.CENT17_cols  <- colnames(tbl.f1)[grep(cent_pattern,colnames(tbl.f1))]
f1_cols      <- unique(c(f.Her2_cols, f.ER_cols, f.PR_cols, f.CENT17_cols))
...
run <- function()
{
   ...
   checkEquals(length(history), 10841)
   checkEquals(as.list(table(unlist(lapply(history, function(e) e["Name"])))), list(`Absent`=26, `Background`= 1020,`Birth`=1088, `Diagnosis`=1088,`Drug`=2314, `Pathology`=1171,`Procedure`=1100, `Progression`=69,  `Radiation`=584, `Status`=1088, `Tests`=1293))
...
# brca surgery at time of Dx & not reported, but other post/pre surgeries in other tables
...
create.DOB.record <- function(patient.id)
{
   ...
   return(list(PatientID=patient.id, PtNum=patient.number, study="TCGAbrca", Name="Birth", Fields= list(date=c(dob), gender=gender, race=race, ethnicity=ethnicity)))
...
test_create.DOB.record <- function()
{
    ...
    x <- create.DOB.record(tcga.ids[1009])
    checkEquals(x, list(PatientID="TCGA.HN.A2NL", PtNum=1009, study="TCGAbrca", Name="Birth", Fields=list(date=NA, gender="female", race=NA, ethnicity=NA)))
...
# t<-  sapply(tbl.drug$pharmaceutical_therapy_drug_name, function(drug){
      if( drug == "[Not Available]" | drug == "[Unkonwn]"){ drug = NA}
      else {
...
test_create.Chemo.record <- function()
{
    ...
    x <- create.Chemo.record(tcga.ids[1]) # no end date
    checkTrue(is.list(x))
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.3C.AAAU", PtNum=1, study=study, Name="Drug", Fields=list(date=c("01/02/2009",NA), therapyType="Chemotherapy", agent="Zoladex", intent=NA, dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
   
   x <- create.Chemo.record("TCGA-C8-A8HR") # recurrence
   checkTrue(is.list(x))
   checkEquals(length(x), 3)
   checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
   checkEquals(names(x[[1]][["Fields"]]), c("date", "therapyType", "agent", "intent", "dose", "units", "totalDose", "totalDoseUnits", "route", "cycle"))
   checkEquals(x[[3]], list(PatientID="TCGA.C8.A8HR", PtNum=711, study=study, Name="Drug", Fields=list(date=c("02/13/2013", "08/26/2013"), therapyType="Chemotherapy", agent="Cyclophosphamide", intent=NA, dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
...
test_create.Diagnosis.record <- function()
{
   ...
   x <- create.Diagnosis.record(tcga.ids[1])
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.3C.AAAU", PtNum=1, study=study, Name="Diagnosis", Fields=list(date="01/01/2004", disease="Breast", siteCode="3C")))
...
test_create.Progression.record <- function()
{
    ...
    x <- create.Progression.record("TCGA-A7-A13E")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "event", "number"))
    checkEquals(x[[1]], list(PatientID="TCGA.A7.A13E", PtNum=135, study=study, Name="Progression", Fields=list(date="07/11/2011", event="Distant Metastasis", number=1)))
    x <- create.Progression.record("TCGA-E2-A152")
    checkEquals(x[[1]], list(PatientID="TCGA.E2.A152", PtNum=815, study=study, Name="Progression", Fields=list(date="02/24/2009", event="Distant Metastasis", number=1)))
    checkEquals(x[[2]], list(PatientID="TCGA.E2.A152", PtNum=815, study=study, Name="Progression", Fields=list(date="03/25/2009", event="Distant Metastasis", number=2)))
...
create.Radiation.record <- function(patient.id)
{
   ...
   for(radEvent in 1:nrow(tbl.radSub)){
      ...
      if(start.radDate != "[Not Available]" & !is.na(diagnosis.date)){
        start.date.unformatted <- diagnosis.date + as.integer(start.radDate)
        start.date <- reformatDate(start.date.unformatted)
      } else { start.date = NA }     
      if(end.radDate != "[Not Available]" & !is.na(diagnosis.date)){
...
test_create.Radiation.record <- function()
{
    ...
    x <- create.Radiation.record("TCGA-HN-A2OB")
    checkTrue(is.list(x))
    checkEquals(x[[1]], list(PatientID="TCGA.HN.A2OB", PtNum=1010, study=study, Name="Radiation", Fields=list(date=c(NA,NA), 
              therapyType="external", intent=NA, target="Primary Tumor Field", totalDose=5000, totalDoseUnits="cGy", numFractions=25)))
   
   x <- create.Radiation.record("TCGA-D8-A1JG") # TotalDose = "42.5 + 10" 
   checkEquals(x[[1]], list(PatientID="TCGA.D8.A1JG", PtNum=730, study=study, Name="Radiation", 
              Fields=list(date=c( "05/12/2010", "06/06/2010"), therapyType="external beam", intent="adjuvant", target="Primary Tumor Field", 
                          totalDose="42.5+10", totalDoseUnits="cGy", numFractions=21)))

   x <- create.Radiation.record("TCGA-BH-A0AU") # radType == "[Unknown]"
   checkEquals(x[[1]], list(PatientID="TCGA.BH.A0AU", PtNum=513, study=study, Name="Radiation", 
              Fields=list(date=c( "03/06/2008", NA), therapyType=NA, intent=NA, target=NA, 
                          totalDose=NA, totalDoseUnits=NA, numFractions=NA)))
} # test_create.Radiation.record
#------------------------------------------------------------------------------------------------------------------------
...
create.Procedure.record <- function(patient.id)
{
    ...
    tbl.ptSub  <- subset(tbl.pt,  bcr_patient_barcode==patient.id &
                        (surgical_procedure_first != "[Not Available]"  | first_surgical_procedure_other!= "[Not Available]") )
    tbl.omfSub <- subset(tbl.omf, bcr_patient_barcode==patient.id & (days_to_surgical_resection != "[Not Available]"))
    
    if( nrow(tbl.ptSub) ==0 && nrow(tbl.omfSub) ==0)
    return(list())
    ...
    result <- vector("list", nrow(tbl.ptSub) + nrow(tbl.omfSub))
    good.records.found <- 0
    
    
    if(nrow(tbl.ptSub)>0){
        for(Event in 1:nrow(tbl.ptSub)){
            Surgery_name <- tbl.ptSub$surgical_procedure_first[Event]
            if(Surgery_name != "[Not Available]"){
                date = NA
                new.event <- list(PatientID=patient.id,
                PtNum=patient.number,
                study=study,
                Name=name,
                Fields = list(date=date, service=NA, name = Surgery_name, site = NA, side = NA, location = NA))
                good.records.found <- good.records.found + 1
                result[[good.records.found]] <- new.event
            }
    ...
    if(nrow(tbl.omfSub)>0){
        for(Event in 1:nrow(tbl.omfSub)){
            ...
            if(side == "[Not Available]" | side == "[Not Applicable]") side = NA
...
test_create.Procedure.record <- function()
{
    ...
    x <- create.Procedure.record(tcga.ids[1])
    checkTrue(is.list(x))
    checkEquals(x[[1]], list(PatientID="TCGA.3C.AAAU", PtNum=1, study=study, Name="Procedure", Fields=list(date=NA, service=NA, name="Modified Radical Mastectomy", site=NA, side=NA, location=NA)))
    
    x <- create.Procedure.record("TCGA-Z7-A8R5")
    checkEquals(length(x),2)
    checkEquals(x[[1]], list(PatientID="TCGA.Z7.A8R5", PtNum=1087, study=study, Name="Procedure", Fields=list(date=NA, service=NA, name="Other", site=NA, side=NA, location=NA)))
    checkEquals(x[[2]], list(PatientID="TCGA.Z7.A8R5", PtNum=1087, study=study, Name="Procedure", Fields=list(date="01/02/1996", service=NA, name="Gross Total Resection", site=NA, side=NA, location=NA)))
    
    x <- create.Procedure.record("TCGA-A7-A13G")
    checkEquals(length(x),3)
    checkEquals(x[[1]], list(PatientID="TCGA.A7.A13G", PtNum=137, study=study, Name="Procedure", Fields=list(date=NA, service=NA, name="Simple Mastectomy", site=NA, side=NA, location=NA)))
    checkEquals(x[[2]], list(PatientID="TCGA.A7.A13G", PtNum=137, study=study, Name="Procedure", Fields=list(date="01/03/2006", service=NA, name=NA, site=NA, side="Left", location=NA)))
    checkEquals(x[[3]], list(PatientID="TCGA.A7.A13G", PtNum=137, study=study, Name="Procedure", Fields=list(date="04/28/1999", service=NA, name=NA, site=NA, side="Left", location=NA)))
} # test_create.Procedure.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Procedure.records <- function(patient.ids)
{
    ...
    #tbl.ptSub <- subset(tbl.pt,   bcr_patient_barcode %in% patient.ids)
    tbl.ptSub  <- subset(tbl.pt,  bcr_patient_barcode %in% patient.ids & (surgical_procedure_first != "[Not Available]"  | first_surgical_procedure_other!= "[Not Available]"))
    
    tbl.omfSub <- subset(tbl.omf, bcr_patient_barcode %in% patient.ids & days_to_surgical_resection != "[Not Available]" & (days_to_surgical_resection != "[Not Available]"))
...
create.Pathology.record <- function(patient.id)
{
   ...
   for(pathEvent in 1:nrow(tbl.pathSub)){
      ...
      if(pathHistology == "[Not Applicable]") pathHistology <- NA
      
      if(collection == "YES"){ collection = "prospective"
      } else if( tbl.pathSub$retrospective_collection[pathEvent]  == "YES"){ collection = "retrospective"
      } else { collection = NA }
   ...
   if(nrow(tbl.omfSub)>0){
         for(omfEvent in 1:nrow(tbl.omfSub)){
          ...
          omf_S.Stage <- tbl.omfSub$pathologic_stage[omfEvent]
          omf_staging.System <- tbl.omfSub$system_version[omfEvent]
          if (omf_T.Stage == "[Not Available]") omf_T.Stage <- NA
          if (omf_N.Stage == "[Not Available]") omf_N.Stage <- NA
          if (omf_M.Stage == "[Not Available]") omf_M.Stage <- NA
          if (omf_S.Stage == "[Not Available]") omf_S.Stage <- NA
          if (omf_staging.System == "[Not Available]") staging.System <- NA
          ...
          if(omfOffset == "[Not Available]" | omfOffset == "[Pending]"){ omf.date = NA
           ...
           new.event <- list(PatientID=patient.id,
                            ...
                            study=study,
                            Name=name,
                            Fields = list(date=omf.date, disease=disease, histology=histology, collection=NA, T.Stage=omf_T.Stage, N.Stage=omf_N.Stage, M.Stage=omf_M.Stage,S.Stage=omf_S.Stage,staging.System=omf_staging.System))
...
test_create.Pathology.record <- function()
{
    ...
    checkEquals(x[[1]], list(PatientID= "TCGA.3C.AAAU", PtNum=1, study=study, Name="Pathology", Fields=list(date="01/01/2004", disease="Breast", histology="Infiltrating Lobular Carcinoma", collection="retrospective", T.Stage="TX",N.Stage="NX",M.Stage="MX",S.Stage="Stage X",staging.System="6th")))
    
    x <- create.Pathology.record("TCGA-AO-A124")
    checkEquals(x[[1]], list(PatientID="TCGA.AO.A124", PtNum=357, study=study, Name="Pathology", Fields=list(date="01/01/2002", disease="Breast", histology="Other  specify", collection="retrospective", T.Stage="T2",N.Stage="N0 (i-)",M.Stage="M0",S.Stage="Stage IIA",staging.System="5th")))

    x <- create.Pathology.record("TCGA-B6-A0I8") #has omf
    checkEquals(x[[1]], list(PatientID="TCGA.B6.A0I8", PtNum=459, study=study, Name="Pathology",Fields=list(date="01/01/1992", disease="Breast", histology="Infiltrating Ductal Carcinoma", collection="retrospective",T.Stage="T1",N.Stage="NX",M.Stage="M0",S.Stage="Stage X",staging.System=NA)))
    checkEquals(x[[2]], list(PatientID="TCGA.B6.A0I8", PtNum=459, study=study, Name="Pathology",Fields=list(date=NA, disease="Breast", histology="Adenocarcinoma, Not Otherwise Specified", collection=NA,T.Stage="T2",N.Stage="N0",M.Stage="M0",S.Stage="Stage II",staging.System="2nd")))
...
create.Absent.record <- function(patient.id)
{
   ...
   if(nrow(tbl.nteSub) == 0 && nrow(tbl.omfSub) == 0 && nrow(tbl.ptSub) == 0 )
       return(list())
   diagnosis.year <- subset(tbl.pt, bcr_patient_barcode==patient.id)$initial_pathologic_dx_year[1]
   if(diagnosis.year == "[Not Available]" || is.na(diagnosis.year)){
       diagnosis.date <- NA
...
test_create.Absent.record <- function()
{
    ...
    x <- create.Absent.record("TCGA-Z7-A8R5") #nte
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "Radiation", "Drug"))
    checkEquals(length(x),2)
    checkEquals(x[[1]], list(PatientID="TCGA.Z7.A8R5", PtNum=1087, study=study, Name="Absent", Fields=list(date="07/01/2005", Radiation="TRUE", Drug="FALSE")))
    checkEquals(x[[2]], list(PatientID="TCGA.Z7.A8R5", PtNum=1087, study=study, Name="Absent", Fields=list(date="01/02/1996", Radiation="TRUE", Drug="TRUE")))
    
    x <- create.Absent.record("TCGA-W8-A86G") #has omf
    checkEquals(x[[1]], list(PatientID="TCGA.W8.A86G", PtNum=1082, study=study, Name="Absent",Fields=list(date="02/24/2013", Radiation="TRUE", Drug="TRUE")))
...
create.all.Absent.records <- function(patient.ids)
{
  ...
  ids <- unique(c(tbl.nteSub$bcr_patient_barcode, tbl.omfSub$bcr_patient_barcode))
...
create.Background.record <- function(patient.id)
{
    ...
    for(event in 1:nrow(tbl.ptSub)){
        menopause.status <- tbl.ptSub$menopause_status[event]
        neoadjuvant.treatment <- tbl.ptSub$history_neoadjuvant_treatment[event]
        if(menopause.status == "[Not Available]" | menopause.status == "[Not Evaluated]"|menopause.status == "[Unknown]") menopause.status <- NA
        ...
        study=study,
        Name=name,
        Fields = list(menopause.status=menopause.status, neoadjuvant.treatment=neoadjuvant.treatment))
        good.records.found <- good.records.found +1
        result[[good.records.found]] <- new.event
    }
    result[1:good.records.found]
...
test_create.Background.record <- function()
{
    ...
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("menopause.status","neoadjuvant.treatment"))
    checkEquals(x[[1]], list(PatientID="TCGA.3C.AAAU", PtNum=1, study=study, Name="Background", Fields=list(menopause.status="Pre (<6 months since LMP AND no prior bilateral ovariectomy AND not on estrogen replacement)", neoadjuvant.treatment="No")))
   
    x <- create.Background.record("TCGA-A2-A0YT") #newoadjuvant.treatment == "Yes"
    checkEquals(x[[1]], list(PatientID="TCGA.A2.A0YT", PtNum=91, study=study, Name="Background", Fields=list(menopause.status="Post (prior bilateral ovariectomy OR >12 mo since LMP with no prior hysterectomy)", neoadjuvant.treatment="Yes")))
...
{
    #1020 good records
    tbl.ptSub <- subset(tbl.pt, bcr_patient_barcode==patient.ids & ((menopause_status != "[unknown]" & menopause_status != "[Not Available]" & menopause_status != "[Not Evaluated]") | history_neoadjuvant_treatment == "Yes"))
...
create.Tests.record <- function(patient.id)
{
    ...
    tbl.ptSub <- subset(tbl.pt, bcr_patient_barcode==patient.id,pt_cols)
    tbl.f1Sub <- subset(tbl.f1, bcr_patient_barcode==patient.id,f1_cols)
    
    if(nrow(tbl.ptSub) == 0 && nrow(tbl.f1Sub) ==0) return(list())
    ...
    if(nrow(tbl.ptSub)!=0){
        for(Event in 1:nrow(tbl.ptSub)){
            Her2_tests_done = Her2_cols[which(!tbl.ptSub[Event,Her2_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            ER_tests_done = ER_cols[which(!tbl.ptSub[Event,ER_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            PR_tests_done = PR_cols[which(!tbl.ptSub[Event,PR_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            CENT17_tests_done = CENT17_cols[which(!tbl.ptSub[Event,CENT17_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            
            
            if(length(Her2_tests_done)>0){
                test = "HER2"
                res = paste(Her2_tests_done, tbl.ptSub[Event,Her2_tests_done],sep=":")
                
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
            if(length(ER_tests_done)>0){
                test = "Estrogen"
                res = paste(ER_tests_done, tbl.ptSub[Event,ER_tests_done],sep=":")
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
            if(length(PR_tests_done)>0){
                test = "Progesterone-receptor"
                res = paste(PR_tests_done, tbl.ptSub[Event,PR_tests_done],sep=":")
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
            if(length(CENT17_tests_done)>0){
                test = "Centromere 17"
                res = paste(CENT17_tests_done, tbl.ptSub[Event,CENT17_tests_done],sep=":")
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
    ...
    if(nrow(tbl.f1Sub)!= 0){
        for(Event in 1:nrow(tbl.f1Sub)){
            f.Her2_tests_done = f.Her2_cols[which(!tbl.f1Sub[Event,f.Her2_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            f.ER_tests_done = f.ER_cols[which(!tbl.f1Sub[Event,f.ER_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            f.PR_tests_done = f.PR_cols[which(!tbl.f1Sub[Event,f.PR_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            f.CENT17_tests_done = f.CENT17_cols[which(!tbl.f1Sub[Event,f.CENT17_cols] %in% c("[Not Available]", "[Not Evaluated]"))]
            if(length(f.Her2_tests_done)>0){
                test = "HER2"
                res = paste(f.Her2_tests_done, tbl.f1Sub[Event,f.Her2_tests_done],sep=":")
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
            if(length(f.ER_tests_done)>0){
                test = "Estrogen"
                res = paste(f.ER_tests_done, tbl.f1Sub[Event,f.ER_tests_done],sep=":")
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
            if(length(f.PR_tests_done)>0){
                test = "Progesterone-receptor"
                res = paste(f.PR_tests_done, tbl.f1Sub[Event,f.PR_tests_done],sep=":")
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
            if(length(f.CENT17_tests_done)>0){
                test = "Centromere 17"
                res = paste(f.CENT17_tests_done, tbl.f1Sub[Event,f.CENT17_tests_done],sep=":")
                new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Test=test, Result=res))
                good.records.found <- good.records.found +1
                result[[good.records.found]] <- new.event
            }
...
{
    print("--- test_create.Absent.record")
    x <- create.Tests.record("TCGA-A2-A0YC") #f1
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("Test", "Result"))
    checkEquals(length(x),8)
    checkEquals(x[[1]], list(PatientID="TCGA.A2.A0YC", PtNum=80, study=study, Name="Tests", Fields=list(Test="HER2",Result=c("her2_fish_status:Negative","her2_and_cent17_cells_count:60","her2_cent17_ratio:1.18","her2_fish_method:Enumerate 60 tumor cells; ratio of HER2/Centromere 17 >= 2.1 is amplified"))))
    checkEquals(x[[3]], list(PatientID="TCGA.A2.A0YC", PtNum=80, study=study, Name="Tests", Fields=list(Test="HER2",Result=c("nte_her2_status:Negative", "nte_her2_positivity_ihc_score:2+"))))
    checkEquals(x[[5]], list(PatientID="TCGA.A2.A0YC", PtNum=80, study=study, Name="Tests", Fields=list(Test="Progesterone-receptor",Result=c("nte_pr_status_by_ihc:Positive","nte_pr_status_ihc__positive:90-99%"))))
...
create.all.Tests.records <- function(patient.ids)
{
    ...
    for(i in 1:nrow(tbl.pt)){
        if(!all(tbl.pt[i,pt_cols] == "[Not Available]" | tbl.pt[i,pt_cols] == "[Not Evaluated]"))
            ids <- c(ids, tbl.pt$bcr_patient_barcode[i])
    }
    
    for(i in 1:nrow(tbl.f1)){
        if(!all(tbl.f1[i,f1_cols] == "[Not Available]" | tbl.f1[i,f1_cols] == "[Not Evaluated]"))
            ids <- c(ids,tbl.f1$bcr_patient_barcode[i])
    }
