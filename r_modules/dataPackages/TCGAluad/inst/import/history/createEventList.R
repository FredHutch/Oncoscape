...
#      choose luad, #cases with data (528 on 2/10/15)
#      choose "Biotab"
#      click "build archive" 
...
#
#      biospecimen_aliquot_luad.txt
#      biospecimen_analyte_luad.txt
#      biospecimen_cqcf_luad.txt                 cqcf: case quality control form
#      biospecimen_diagnostic_slides_luad.txt
#      biospecimen_normal_control_luad.txt
#      biospecimen_portion_luad.txt
#      biospecimen_protocol_luad.txt
#      biospecimen_sample_luad.txt
#      biospecimen_shipment_portion_luad.txt
#      biospecimen_slide_luad.txt
#      biospecimen_tumor_sample_luad.txt
#      clinical_drug_luad.txt
#      clinical_follow_up_v1.0_luad.txt
#      clinical_follow_up_v1.0_nte_luad.txt
#      clinical_nte_luad.txt
#      clinical_omf_v4.0_luad.txt
#      clinical_patient_luad.txt
#      clinical_radiation_luad.txt
#
...
#   clinical_radiation_luad.txt
#   clinical_follow_up_v1.0_luad.txt
#   clinical_follow_up_v1.0_nte_luad.txt      nte: new tumor event
#   clinical_nte_luad.txt
#   clinical_omf_v4.0_luad.txt                omf: other malignancy form
#
#------------------------------------------------------------------------------------------------------------------------
...
currDir <- getwd()
setwd("../../../../RawData/TCGAluad/Clinical_3-25-15/")

study="TCGAluad"

tbl.pt <- read.table("clinical_patient_luad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
tbl.drug <- read.table("clinical_drug_luad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.drug <- tbl.drug[3:nrow(tbl.drug),]
tbl.rad <- read.table("clinical_radiation_luad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.rad <- tbl.rad[3:nrow(tbl.rad),]
tbl.f1 <- read.table("clinical_follow_up_v1.0_luad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f1 <- tbl.f1[3:nrow(tbl.f1),]
#tbl.f2 <- read.table("clinical_follow_up_v1.0_nte_luad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
#tbl.f2 <- tbl.f2[3:nrow(tbl.f2),]
tbl.nte <- read.table("clinical_nte_luad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.nte <- tbl.nte[3:nrow(tbl.nte),]
tbl.omf <- read.table("clinical_omf_v4.0_luad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
run <- function()
{
   ...
   checkEquals(length(history), 5502)
   checkEquals(as.list(table(unlist(lapply(history, function(e) e["Name"])))), list(`Absent`=46, `Background`= 508,`Birth`=520, `Diagnosis`=520,`Drug`=354,`Encounter`=1530, `Pathology`=625,`Procedure`=99, `Progression`=30,  `Radiation`=104, `Status`=520, `Tests`=646))
...
# luad surgery at time of Dx & not reported, but other post/pre surgeries in other tables
...
create.DOB.record <- function(patient.id)
{
   ...
   return(list(PatientID=patient.id, PtNum=patient.number, study="TCGAluad", Name="Birth", Fields= list(date=c(dob), gender=gender, race=race, ethnicity=ethnicity)))
...
test_create.DOB.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.05.4405", PtNum=15, study="TCGAluad", Name="Birth", Fields=list(date="06/03/1931", gender="female", race=NA, ethnicity=NA)))
...
test_create.Chemo.record <- function()
{
    ...
    x <- create.Chemo.record("TCGA-75-7030")
    checkTrue(is.list(x))
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.75.7030", PtNum=336, study=study, Name="Drug", Fields=list(date=c(NA,NA), therapyType="Chemotherapy", agent="Vinorelbine", intent="adjuvant", dose=46, units="mg/day", totalDose=552, totalDoseUnits="mg", route="Intravenous (IV)", cycle=4)))
    checkEquals(x[[2]], list(PatientID="TCGA.75.7030", PtNum=336, study=study, Name="Drug", Fields=list(date=c(NA,NA), therapyType="Chemotherapy", agent="Cisplatin", intent="adjuvant"  , dose=92, units="mg/day", totalDose=736,  totalDoseUnits="mg", route="Intravenous (IV)", cycle=4)))
    

   x <- create.Chemo.record("TCGA-95-7039")  #no start date
   checkEquals(length(x), 1)
   checkEquals(x[[1]], list(PatientID="TCGA.95.7039", PtNum=432, study=study, Name="Drug", Fields=list(date=c(NA,NA), therapyType=NA, agent=NA, intent=NA  , dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
   x <- create.Chemo.record("TCGA-05-4424")  # no end date
   checkEquals(length(x), 1)
   checkEquals(x[[1]], list(PatientID="TCGA.05.4424", PtNum=22, study=study, Name="Drug", Fields=list(date=c("11/30/2009", NA), therapyType="Immunotherapy", agent="Erlotinib", intent=NA, dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
   x <- create.Chemo.record("TCGA-38-7271") # recurrence
   checkEquals(length(x), 3)
   checkEquals(x[[1]], list(PatientID="TCGA.38.7271", PtNum=49, study=study, Name="Drug", Fields=list(date=c("11/26/2006", "01/07/2007"), therapyType="Chemotherapy", agent="Cisplatin", intent="recurrence", dose=30, units="mg/m2", totalDose=324, totalDoseUnits="mg", route="Intravenous (IV)", cycle=3)))
   x <- create.Chemo.record("TCGA-05-4245") # omf chemo
   checkEquals(x[[1]], list(PatientID="TCGA.05.4245", PtNum=2, study=study, Name="Drug", Fields=list(date=c(NA, NA), therapyType=NA, agent=NA, intent="Prior Malignancy" , dose=NA, units=NA, totalDose=NA, totalDoseUnits=NA, route=NA, cycle=NA)))
...
test_create.Diagnosis.record <- function()
{
   ...
   x <- create.Diagnosis.record(tcga.ids[1])
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.05.4244", PtNum=1, study=study, Name="Diagnosis", Fields=list(date="01/01/2009", disease="Lung", siteCode="05")))
...
test_create.status.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.05.4244", PtNum=1, study=study, Name="Status", Fields=list(date="01/01/2009", status="Alive", tumorStatus="tumor free")))
	x <- create.status.record("TCGA-49-4486")
    checkEquals(x, list(PatientID="TCGA.49.4486", PtNum=100, study=study, Name="Status", Fields=list(date="05/07/1998", status="Dead", tumorStatus="with tumor")))
...
test_create.Progression.record <- function()
{
    ...
    x <- create.Progression.record("TCGA-49-AAQV")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "event", "number"))
    checkEquals(x[[1]], list(PatientID="TCGA.49.AAQV", PtNum=118, study=study, Name="Progression", Fields=list(date="05/24/2013", event="Distant Metastasis", number=1)))
    x <- create.Progression.record("TCGA-MP-A4TE")
    checkEquals(x[[1]], list(PatientID="TCGA.MP.A4TE", PtNum=504, study=study, Name="Progression", Fields=list(date="08/15/2010", event="Locoregional Recurrence", number=1)))
...
test_create.Radiation.record <- function()
{
    ...
    x <- create.Radiation.record(tcga.ids[5])
    checkTrue(is.list(x))
   checkEquals(x[[1]], list(PatientID="TCGA.05.4382", PtNum=5, study=study, Name="Radiation", Fields=list(date=c("01/01/2010", "01/29/2010"), therapyType="external", intent=NA, target="Distant Recurrence", totalDose=NA, totalDoseUnits="Gy", numFractions=NA)))
   x <- create.Radiation.record("TCGA-44-7669")  # cCi
   checkEquals(x[[1]], list(PatientID="TCGA.44.7669", PtNum=86, study=study, Name="Radiation", Fields=list(date=c( "04/20/2011", "04/20/2011"), therapyType="cyber knife", intent="progression", target="Distant site", totalDose=2000, totalDoseUnits="cGy", numFractions=1)))
   checkEquals(x[[2]], list(PatientID="TCGA.44.7669", PtNum=86, study=study, Name="Radiation", Fields=list(date=c("06/11/2011", "06/17/2011"), therapyType="external beam", intent="palliative", target="Distant Recurrence", totalDose=2000, totalDoseUnits="cGy", numFractions=5)))

   x <- create.Radiation.record("TCGA-49-AAR4")  #in totalDosage is 4,500 cGy
   checkEquals(x[[1]], list(PatientID="TCGA.49.AAR4", PtNum=122, study=study, Name="Radiation", Fields=list(date=c( "04/29/2006", "06/03/2006"), therapyType="external", intent=NA, target="Primary Tumor Field", totalDose=4500, totalDoseUnits="cGy", numFractions=NA)))
} # test_create.Radiation.record
#------------------------------------------------------------------------------------------------------------------------
...
test_create.Encounter.record <- function()
{
    ...
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("type", "kps", "ecog", "date", "fev1.ratio", "fev1.percent","carbon.monoxide.diffusion",
                                             "systolic","diastolic","height","weight","BSA","BMI","ZubrodScore"))
...
create.Procedure.record <- function(patient.id)
{
    ...
    if(nrow(tbl.omfSub)>0){
        for(Event in 1:nrow(tbl.omfSub)){
            ...
            if(date == "[Not Available]" & date == "[Not Applicable]"){ date = NA
...
test_create.Procedure.record <- function()
{
    ...
    x <- create.Procedure.record(tcga.ids[2])
    checkTrue(is.list(x))
    checkEquals(x[[1]], list(PatientID="TCGA.05.4245", PtNum=2, study=study, Name="Procedure", Fields=list(date="01/31/2006", service=NA, name=NA, site=NA, side=NA, location=NA)))
    
    x <- create.Procedure.record("TCGA-MP-A4T9")
    checkEquals(length(x), 2)
    checkEquals(names(x[[1]]), c("PatientID", "PtNum","study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "service", "name", "site", "side", "location"))
    checkEquals(x[[2]], list(PatientID= "TCGA.MP.A4T9", PtNum=500, study=study, Name="Procedure", Fields=list(date= "06/09/2009", service=NA, name=NA, site="locoregional", side=NA, location=NA)))
    checkEquals(x[[1]], list(PatientID= "TCGA.MP.A4T9", PtNum=500, study=study, Name="Procedure", Fields=list(date="07/27/2008", service=NA, name=NA, site="metastasis", side=NA, location=NA)))
...
test_create.Absent.record <- function()
{
    ...
    x <- create.Absent.record("TCGA-62-A46O") #nte
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "Radiation", "Drug"))
    checkEquals(x[[1]], list(PatientID="TCGA.62.A46O", PtNum=261, study=study, Name="Absent", Fields=list(date="08/01/2008", Radiation="TRUE", Drug="TRUE")))
    
    #x <- create.Absent.record("TCGA-38-4630") #has omf
    #checkEquals(x[[1]], list(PatientID="TCGA.38.4630", PtNum=45, study=study, Name="Absent",Fields=list(date=NA, Radiation=NA, Drug=NA)))
...
create.all.Absent.records <- function(patient.ids)
{
   ...
   #tbl.ptSub <- subset(tbl.pt, pulmonary_function_test_indicator == "NO")
...
test_create.Background.record <- function()
{
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.49.4486", PtNum=100, study=study, Name="Background", Fields=list(tobacco.usages=c("1932","1967"), smoking.status=NA, num.packs.years="17.5",num.packs.day=NA, alcohol.usage.yrs=NA, alcohol.amount=NA, neoadjuvant.treatment="No")))
   
    x <- create.Background.record("TCGA-38-4629") #no tobacco usage starting date
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("tobacco.usages","smoking.status","num.packs.years","num.packs.day",
    "alcohol.usage.yrs", "alcohol.amount","neoadjuvant.treatment"))
    checkEquals(x[[1]], list(PatientID="TCGA.38.4629", PtNum=44, study=study, Name="Background", Fields=list(tobacco.usages=c(NA,"2003"), smoking.status=NA, num.packs.years="100",num.packs.day=NA, alcohol.usage.yrs=NA, alcohol.amount=NA, neoadjuvant.treatment="No")))
