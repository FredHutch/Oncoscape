...
#      choose prad, #cases with data (528 on 2/10/15)
#      choose "Biotab"
#      click "build archive"
...
#
#      biospecimen_aliquot_prad.txt
#      biospecimen_analyte_prad.txt
#      biospecimen_cqcf_prad.txt                 cqcf: case quality control form
#      biospecimen_diagnostic_slides_prad.txt
#      biospecimen_normal_control_prad.txt
#      biospecimen_portion_prad.txt
#      biospecimen_protocol_prad.txt
#      biospecimen_sample_prad.txt
#      biospecimen_shipment_portion_prad.txt
#      biospecimen_slide_prad.txt
#      biospecimen_tumor_sample_prad.txt
#      clinical_drug_prad.txt
#      clinical_follow_up_v1.0_prad.txt
#      clinical_follow_up_v1.0_nte_prad.txt
#      clinical_nte_prad.txt
#      clinical_omf_v4.0_prad.txt
#      clinical_patient_prad.txt
#      clinical_radiation_prad.txt
#
...
#   clinical_radiation_prad.txt
#   clinical_follow_up_v1.0_prad.txt
#   clinical_follow_up_v1.0_nte_prad.txt      nte: new tumor event
#   clinical_nte_prad.txt
#   clinical_omf_v4.0_prad.txt                omf: other malignancy form
#
#------------------------------------------------------------------------------------------------------------------------
...
currDir <- getwd()
setwd("../../../../RawData/TCGAprad/Clinical_7-15-15/")

study="TCGAprad"

tbl.pt <- read.table("clinical_patient_prad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
tbl.drug <- read.table("clinical_drug_prad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.drug <- tbl.drug[3:nrow(tbl.drug),]
tbl.rad <- read.table("clinical_radiation_prad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.rad <- tbl.rad[3:nrow(tbl.rad),]
tbl.f1 <- read.table("clinical_follow_up_v1.0_prad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f1 <- tbl.f1[3:nrow(tbl.f1),]
#tbl.f2 <- read.table("clinical_follow_up_v1.0_nte_prad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
#tbl.f2 <- tbl.f2[3:nrow(tbl.f2),]
tbl.nte <- read.table("clinical_nte_prad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.nte <- tbl.nte[3:nrow(tbl.nte),]
tbl.omf <- read.table("clinical_omf_v4.0_prad.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
create.DOB.record <- function(patient.id)
{
    ...
    return(list(PatientID=patient.id, PtNum=patient.number, study="TCGAprad", Name="Birth", Fields= list(date=c(dob), gender=gender, race=race, ethnicity=ethnicity)))
...
test_create.DOB.record <- function()
{
    ...
    x <- create.DOB.record(tcga.ids[367])
    checkEquals(x, list(PatientID="TCGA.V1.A8MF", PtNum=367, study="TCGAprad", Name="Birth", Fields=list(date=NA, gender="male", race=NA, ethnicity=NA)))
...
test_create.status.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.2A.A8VL", PtNum=1, study=study, Name="Status", Fields=list(date="09/14/2011", status="Alive", tumorStatus="tumor free")))
    x <- create.status.record(tcga.ids[367])
    checkEquals(x, list(PatientID="TCGA.V1.A8MF", PtNum=367, study=study, Name="Status", Fields=list(date=NA, status="Alive", tumorStatus=NA)))
} # test_create.status.record
#------------------------------------------------------------------------------------------------------------------------
...
test_create.Progression.record <- function()
{
    ...
    x <- create.Progression.record("TCGA-CH-5791")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "event", "number"))
    checkEquals(x[[1]], list(PatientID="TCGA.CH.5791", PtNum=41, study=study, Name="Progression", Fields=list(date="02/01/2007", event="New Primary Tumor", number=1)))
    checkEquals(x[[2]], list(PatientID="TCGA.CH.5791", PtNum=41, study=study, Name="Progression", Fields=list(date="09/01/2008", event="New Primary Tumor", number=2)))
    x <- create.Progression.record("TCGA-YL-A9WK")
    checkEquals(x[[1]], list(PatientID="TCGA.YL.A9WK", PtNum=470, study=study, Name="Progression", Fields=list(date="10/06/2010", event="Biochemical evidence of disease;Biochemical evidence of disease;Biochemical evidence of disease", number=1)))
...
test_create.Radiation.record <- function()
{
    ...
    x <- create.Radiation.record("TCGA-EJ-5524")
    checkTrue(is.list(x))
    checkEquals(x[[1]], list(PatientID="TCGA.EJ.5524", PtNum=70, study=study, Name="Radiation", Fields=list(date=c("04/23/2010","04/23/2010"), therapyType=NA, intent=NA, target="Local Recurrence", totalDose=NA, totalDoseUnits=NA, numFractions=NA)))
} # test_create.Radiation.record
#------------------------------------------------------------------------------------------------------------------------
...
test_create.Procedure.record <- function()
{
    ...
    x <- create.Procedure.record("TCGA-CH-5763")
    checkEquals(length(x), 1)
    checkEquals(names(x[[1]]), c("PatientID", "PtNum","study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "service", "name", "site", "side", "location"))
    checkEquals(x[[1]], list(PatientID= "TCGA.CH.5763", PtNum=29, study=study, Name="Procedure", Fields=list(date= "10/02/2007", service=NA, name=NA, site=NA, side=NA, location=NA)))
    x <- create.Procedure.record("TCGA-KK-A8IB")
    checkEquals(x[[1]], list(PatientID= "TCGA.KK.A8IB", PtNum=338, study=study, Name="Procedure", Fields=list(date= "02/25/2006", service=NA, name=NA, site=NA, side=NA, location=NA)))
} # test_create.Procedure.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Procedure.records <- function(patient.ids)
{
    ...
    ids <- unique(tbl.omfSub$bcr_patient_barcode)
...
test_create.Pathology.record <- function()
{
    ...
    checkEquals(x[[1]], list(PatientID= "TCGA.CH.5753", PtNum=25, study=study, Name="Pathology", Fields=list(date="01/01/2008", disease="Prostate", histology="Prostate Adenocarcinoma Acinar Type", collection="prospective", T.Stage=NA,N.Stage=NA,M.Stage="M0",S.Stage=NA,staging.System=NA)))
    checkEquals(x[[2]], list(PatientID= "TCGA.CH.5753", PtNum=25, study=study, Name="Pathology", Fields=list(date=NA, disease="Lymph node(s)", histology="Other, specify", collection=NA, T.Stage=NA,N.Stage=NA,M.Stage=NA,S.Stage=NA,staging.System=NA)))
} # test_create.Pathology.record
#------------------------------------------------------------------------------------------------------------------------
...
test_create.Absent.record <- function()
{
    ...
    x <- create.Absent.record("TCGA-KK-A7B4") #nte & omf
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "Radiation", "Drug"))
    checkEquals(x[[1]], list(PatientID="TCGA.KK.A7B4", PtNum=330, study=study, Name="Absent", Fields=list(date="05/10/1999", Radiation="FALSE", Drug="TRUE")))
    
    x <- create.Absent.record("TCGA-EJ-A6RC") #nte & omf
    checkEquals(x[[2]], list(PatientID="TCGA.EJ.A6RC", PtNum=126, study=study, Name="Absent",Fields=list(date=NA, Radiation=NA, Drug="TRUE")))
    x <- create.Absent.record("TCGA-2A-A8VL")
} # test_create.Absent.record
#------------------------------------------------------------------------------------------------------------------------
...
create.Background.record <- function(patient.id)
{
    ...
    return(list(PatientID=patient.id, PtNum=patient.number, study=study, Name=name, Fields = list(History = history, neoadjuvant.treatment=neoadjuvant.treatment,First_Tx_Outcome=First_Tx_Outcome,tobacco.usages=NA, smoking.status=NA, num.packs.years=NA,num.packs.day=NA, alcohol.usage.yrs=NA, alcohol.amount=NA, neoadjuvant.treatment=NA,num.children=NA, carcinogen.exposure=NA, occupation=NA,marital.status=NA, vietnam.vet=NA)))
...
test_create.Background.record <- function()
{
    ...
    "alcohol.amount","neoadjuvant.treatment","num.children","carcinogen.exposure","occupation",
    "marital.status","vietnam.vet"))
    checkEquals(x, list(PatientID="TCGA.EJ.7789", PtNum=100, study="TCGAprad", Name="Background", Fields=list(History=list(YES=c("history of other malignancy"),NO=NA),neoadjuvant.treatment="No", First_Tx_Outcome=NA,tobacco.usages=NA, smoking.status=NA, num.packs.years=NA,num.packs.day=NA, alcohol.usage.yrs=NA, alcohol.amount=NA, neoadjuvant.treatment=NA, num.children=NA,carcinogen.exposure=NA,occupation=NA,marital.status=NA,vietnam.vet=NA)))
...
{
    tbl.ptSub <- subset(tbl.pt, bcr_patient_barcode==patient.id & (psa_most_recent_results != "[Not Available]" |
    bone_scan_results != "[Not Available]" | ct_scan_ab_pelvis_indicator != "[Not Available]"| ct_scan_ab_pelvis_indicator != "[Unknown]" | mri_at_diagnosis == "YES"))
    ...
    for(Event in 1:nrow(tbl.ptSub)){
        if(tbl.ptSub[Event,]$psa_most_recent_results != "[Not Available]"){
            test = "PSA"
            psa.Offset <- tbl.ptSub$days_to_psa_most_recent
            if(psa.Offset != "[Not Available]" & (!is.na(diagnosis.date))){
                psa.Offset <-   as.integer(psa.Offset)
                psa.date <- reformatDate(format(diagnosis.date + psa.Offset))
            }else{
                psa.date <- NA
            }
            res <- tbl.ptSub$psa_most_recent_results
            if(res == "[Not Available]"){ res <- NA}
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=psa.date, Type="PSA", Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
        if(tbl.ptSub[Event,]$bone_scan_results != "[Not Available]"){
            test="bone scan"
            bone.scan.Offset <- tbl.ptSub$days_to_bone_scan
            if(bone.scan.Offset != "[Not Available]" & (!is.na(diagnosis.date))){
                bone.scan.Offset <-   as.integer(bone.scan.Offset)
                bone.scan.date <- reformatDate(format(diagnosis.date +  bone.scan.Offset))
            }else{
                bone.scan.date <- NA
            }
            res <- tbl.ptSub$bone_scan_results
            if(res == "[Not Available]"){ res <- NA}
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=bone.scan.date,Type="bone scan",Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
        if(tbl.ptSub[Event,]$ct_scan_ab_pelvis_indicator != "[Not Available]"| tbl.ptSub[Event,]$ct_scan_ab_pelvis_indicator != "[Unknown]"){
            test="CT scan"
            ct.scan.Offset <- tbl.ptSub$days_to_ct_scan_ab_pelvis
            if(ct.scan.Offset != "[Not Available]" & (!is.na(diagnosis.date))){
                ct.scan.Offset <-   as.integer(ct.scan.Offset)
                ct.scan.date <- reformatDate(format(diagnosis.date +  ct.scan.Offset))
            }else{
                ct.scan.date <- NA
            }
            res <- tbl.ptSub$ct_scan_ab_pelvis_results
            if(res == "[Not Available]"){ res <- NA}
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=ct.scan.date,Type="CT scan",Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
        if(tbl.ptSub[Event,]$mri_at_diagnosis == "YES"){
            test="MRI"
            mri.Offset <- tbl.ptSub$days_to_mri
            if(mri.Offset != "[Not Available]" & (!is.na(diagnosis.date))){
                mri.Offset <-   as.integer(mri.Offset)
                mri.date <- reformatDate(format(diagnosis.date +  mri.Offset))
            }else{
                mri.date <- NA
            }
            res <- tbl.ptSub$mri_results
            if(res == "[Not Available]"){ res <- NA}
            new.event <- list(PatientID=patient.id, PtNum=patient.number,study=study, Name=name, Fields = list(Date=mri.date,Type="MRI",Test=test, Result=res))
            good.records.found <- good.records.found +1
            result[[good.records.found]] <- new.event
        }
    }
...
{
    tbl.ptSub <- subset(tbl.pt, bcr_patient_barcode %in% patient.ids &  (psa_most_recent_results != "[Not Available]" |
    bone_scan_results != "[Not Available]" | ct_scan_ab_pelvis_indicator != "[Not Available]"| ct_scan_ab_pelvis_indicator != "[Unknown]" | mri_at_diagnosis == "YES"))
