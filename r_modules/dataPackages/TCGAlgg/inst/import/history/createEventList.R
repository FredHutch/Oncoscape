...
#      choose lgg, #cases with data (459 on 2/10/15)
#      choose "Biotab"
#      click "build archive" 
...
#
#      biospecimen_aliquot_lgg.txt
#      biospecimen_analyte_lgg.txt
#      biospecimen_cqcf_lgg.txt                 cqcf: case quality control form
#      biospecimen_diagnostic_slides_lgg.txt
#      biospecimen_normal_control_lgg.txt
#      biospecimen_portion_lgg.txt
#      biospecimen_protocol_lgg.txt
#      biospecimen_sample_lgg.txt
#      biospecimen_shipment_portion_lgg.txt
#      biospecimen_slide_lgg.txt
#      biospecimen_tumor_sample_lgg.txt
#      clinical_drug_lgg.txt
#      clinical_follow_up_v1.0_lgg.txt
#      clinical_follow_up_v1.0_nte_lgg.txt
#      clinical_nte_lgg.txt
#      clinical_omf_v4.0_lgg.txt
#      clinical_patient_lgg.txt
#      clinical_radiation_lgg.txt
#
...
#   clinical_radiation_lgg.txt
#   clinical_follow_up_v1.0_lgg.txt
#   clinical_follow_up_v1.0_nte_lgg.txt      nte: new tumor event
#   clinical_nte_lgg.txt
#   clinical_omf_v4.0_lgg.txt                omf: other malignancy form
#
#------------------------------------------------------------------------------------------------------------------------
...
currDir <- getwd()
setwd("../../../../RawData/TCGAlgg/Clinical_2-10-15/")

study="TCGAlgg"

tbl.pt <- read.table("nationwidechildrens.org_clinical_patient_lgg.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
tbl.drug <- read.table("nationwidechildrens.org_clinical_drug_lgg.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.drug <- tbl.drug[3:nrow(tbl.drug),]
tbl.rad <- read.table("nationwidechildrens.org_clinical_radiation_lgg.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.rad <- tbl.rad[3:nrow(tbl.rad),]
tbl.f1 <- read.table("nationwidechildrens.org_clinical_follow_up_v1.0_lgg.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
tbl.f1 <- tbl.f1[3:nrow(tbl.f1),]
tbl.nte <- read.table("nationwidechildrens.org_clinical_nte_lgg.txt", quote="", sep="\t", header=TRUE, as.is=TRUE)
...
test_create.DOB.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.CS.6290", PtNum=1, study=study, Name="Birth", Fields=list(date="01/23/1977", gender="male", race=NA, ethnicity=NA)))
    x <- create.DOB.record(tcga.ids[425])
    checkEquals(x, list(PatientID="TCGA.W9.A837", PtNum=425, study=study, Name="Birth", Fields=list(date=NA, gender="male", race="white", ethnicity="not hispanic or latino")))
...
# t<-  sapply(tbl.drug$pharmaceutical_therapy_drug_name, function(drug){ 
      ...
      if( drug == "Acctuane" || drug=="Isotrectinoin (acccutane)" | drug == "Isotretinoin"){ drug =  "Accutane"}
      ...
      if( drug == "Carmustin" | drug == "Carmustine (BCNU)" | drug == "Carmustine BCNU" | drug == "BCNU (carmustine)" | drug == "BCNU (Carmustine)") { drug = "Carmustine"}
      ...
      if( drug == "Topecan" | drug == "Toptecan"){ drug ="Topotecan" }
      ...
      if( drug == "Carboplatin 5 AUC"){ drug = "Carboplatin"}
      if( drug == "CDR0000654697") { drug = "[Not Available]"}
...
test_create.Chemo.record <- function()
{
    ...
    x <- create.Chemo.record(tcga.ids[1])
    checkTrue(is.list(x))
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.CS.6290", PtNum=1, study=study, Name="Drug", Fields=list(date=c(NA,"05/20/2010"), therapyType="Chemotherapy", agent="Temodar", intent="adjuvant"  , dose=400, units="mg"   , totalDose=NA, totalDoseUnits="mg"   , route="Oral", cycle=12)))

   x <- create.Chemo.record("TCGA-DU-6402") 
   checkEquals(length(x), 1)
   checkEquals(x[[1]], list(PatientID="TCGA.DU.6402", PtNum=20, study=study, Name="Drug", Fields=list(date=c("04/28/1998", "05/03/1998"), therapyType="Chemotherapy", agent="Temozolomide", intent="progression"  , dose=100, units="mg/m2", totalDose=200, totalDoseUnits="mg", route="Oral", cycle=1)))
...
#   $Name   [1] "Diagnosis"
#   $date   [1] "01/01/2006"
#   $disease   [1] "Central nervous system"
#
...
test_create.status.record <- function()
{
    ...
    checkEquals(x, list(PatientID="TCGA.CS.6290", PtNum=1, study=study, Name="Status", Fields=list(date="07/01/2010", status="Alive", tumorStatus="with tumor")))
...
create.Progression.record <- function(patient.id)
{
  ...
  if(nrow(tbl.nte.rows)>0) { 
     nte <- cbind(NA, tbl.nte.rows[,c("new_tumor_event_dx_days_to")])
     colnames(nte) <- c("new_neoplasm_event_type","new_tumor_event_dx_days_to")
...
test_create.Progression.record <- function()
{
    ...
    x <- create.Progression.record("TCGA-DU-6407")
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum", "study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "event", "number"))
    checkEquals(x[[1]], list(PatientID="TCGA.DU.6407", PtNum=25, study=study, Name="Progression", Fields=list(date="07/26/2007", event=NA, number=1)))
    x <- create.Progression.record("TCGA-DU-5852")
    checkEquals(x[[1]], list(PatientID="TCGA.DU.5852", PtNum=4, study=study, Name="Progression", Fields=list(date="01/25/2010", event=NA, number=1)))
    x <- create.Progression.record("TCGA-HT-8564")  #in nte table
    checkEquals(length(x), 1)
    checkEquals(x[[1]], list(PatientID="TCGA.HT.8564", PtNum=188, study=study, Name="Progression", Fields=list(date="04/30/2012", event=NA, number=1)))
    x <- create.Progression.record("TCGA-FG-5963")  #2 progression events
    checkEquals(length(x), 2)
    checkEquals(x[[1]], list(PatientID="TCGA.FG.5963", PtNum=27, study=study, Name="Progression", Fields=list(date="01/31/2010", event=NA, number=1)))
    checkEquals(x[[2]], list(PatientID="TCGA.FG.5963", PtNum=27, study=study, Name="Progression", Fields=list(date="05/13/2010", event=NA, number=2)))
...
{
      # 262 good rows
   tbl.good <- subset(tbl.f1, bcr_patient_barcode %in% patient.ids & new_tumor_event_dx_days_to != "[Not Available]")
...
#    $disease     "Brain"
    ...
    for(omfEvent in 1:nrow(tbl.omfSub)){
       start.radDate <- tbl.omfSub$days_to_radiation_therapy_start[omfEvent]
      if(start.radDate != "[Not Available]"){
        start.date.unformatted <- diagnosis.date + as.integer(start.radDate)
        start.date <- reformatDate(start.date.unformatted)
       }
...
test_create.Radiation.record <- function()
{
    ...
    checkTrue(is.list(x))
    checkEquals(names(x[[1]]), c("PatientID", "PtNum","study", "Name", "Fields"))
    checkEquals(x[[1]], list(PatientID="TCGA.CS.6290", PtNum=1, study=study, Name="Radiation", Fields=list(date=c(NA, "04/26/2009"), therapyType="external beam", intent="adjuvant", target="Primary Tumor Field", totalDose=NA, totalDoseUnits=NA, numFractions=NA)))

    x <- create.Radiation.record(tcga.ids[57])
    checkTrue(is.list(x))
    checkEquals(length(x), 0)

   x <- create.Radiation.record("TCGA-FG-A4MY")  
   checkEquals(x[[1]], list(PatientID="TCGA.FG.A4MY", PtNum=200, study=study, Name="Radiation", Fields=list(date=c("02/06/2012", "03/18/2012"), therapyType="external", intent=NA, target="Primary Tumor Field", totalDose=5700, totalDoseUnits="cGy", numFractions=30)))
 
   x<- create.Radiation.record("TCGA-HT-A619")  
   checkEquals(x[[1]], list(PatientID="TCGA.HT.A619", PtNum=260, study=study, Name="Radiation", Fields=list(date=c("08/19/2001", NA), therapyType=NA, intent="Prior Malignancy", target="Locoregional, at primary tumor site: yes", totalDose=NA, totalDoseUnits=NA, numFractions=NA)))
...
test_create.Encounter.record <- function()
{
    ...
    checkEquals(x[[1]], list(PatientID="TCGA.CS.6290", PtNum=1, study="TCGAlgg",  Name="Encounter", Fields=list(type="Pre-Operative", kps=90, ecog=1)))
    checkEquals(x[[2]], list(PatientID="TCGA.CS.6290", PtNum=1, study="TCGAlgg", Name="Encounter", Fields=list(type=NA, kps=NA, ecog=NA)))
    x <- create.Encounter.record("TCGA-FG-6691") #pre-operative timing
    checkEquals(x[[1]], list(PatientID="TCGA.FG.6691", PtNum=49, study=study, Name="Encounter", Fields=list(type="Pre-Operative", kps=100, ecog=NA)))
    checkEquals(x[[2]], list(PatientID="TCGA.FG.6691", PtNum=49, study=study, Name="Encounter", Fields=list(type="Pre-Operative", kps=90, ecog=NA)))
    checkEquals(x[[3]], list(PatientID="TCGA.FG.6691", PtNum=49, study=study, Name="Encounter", Fields=list(type="Adjuvant therapy", kps=80, ecog=NA)))
    checkEquals(x[[4]], list(PatientID="TCGA.FG.6691", PtNum=49, study=study, Name="Encounter", Fields=list(type=NA, kps=NA, ecog=NA)))
 
} # test_create.Encounter.record
#------------------------------------------------------------------------------------------------------------------------
create.all.Encounter.records <- function(patient.ids)
{
   tbl.encSub <- subset(tbl.pt, bcr_patient_barcode %in% patient.ids)
   tbl.nteSub <- subset(tbl.f1, bcr_patient_barcode %in% patient.ids)
   ids <- unique(c(tbl.encSub$bcr_patient_barcode,tbl.nteSub$bcr_patient_barcode) ) 
...
create.Procedure.record <- function(patient.id)
{
   ...
   tbl.f1Sub  <- subset(tbl.f1,  bcr_patient_barcode==patient.id & (new_tumor_event_surgery_days_to_loco != "[Not Available]" | new_tumor_event_surgery_days_to_met != "[Not Available]"))
   tbl.nteSub <- subset(tbl.nte, bcr_patient_barcode==patient.id & (new_tumor_event_surgery_days_to_loco != "[Not Available]" | new_tumor_event_surgery_days_to_met != "[Not Available]"))
   tbl.omfSub <- subset(tbl.omf, bcr_patient_barcode==patient.id & days_to_surgical_resection != "[Not Available]")

   if( nrow(tbl.ptSub) ==0 && nrow(tbl.f1Sub) ==0 && nrow(tbl.nteSub) ==0 && nrow(tbl.omfSub) ==0)
       return(list())  
   ...
   for(Event in 1:nrow(tbl.ptSub)){
   	  side <- tbl.ptSub$laterality[Event]
   	  if(side == "[Not Available]") side = NA
   	  site <- tbl.ptSub$tumor_site; 
   	  if(site == "locoregional disease") site = "locoregional"
   	  if(site == "progression of disease") site = "progression"
   	  local <- tbl.ptSub$supratentorial_localization
   	  if(local == "Not listed in Medical Record" || local == "[Not Available]") local = NA;
   	  if(!is.na(local)) site <- paste(site, local, sep=": ")
   	  
      new.event <- list(PatientID=patient.id,
                        PtNum=patient.number,
                        study=study,
                        Name=name,
                        Fields = list(date=NA, service=NA, name = NA, site = site, side = side))
      good.records.found <- good.records.found + 1
      result[[good.records.found]] <- new.event
      }} # for encEvent
   ...
   for(Event in 1:nrow(tbl.f1Sub)){
      ...
      if(date != "[Not Available]"){ 
         ...
         new.event <- list(PatientID=patient.id,
                        ...
                        study=study,
                        Name=name,
                        Fields = list(date=date, service=NA, name = NA, site = "locoregional", side = NA))
         good.records.found <- good.records.found + 1
         result[[good.records.found]] <- new.event
      }
      ...
      if(date != "[Not Available]"){ 
         ...
         new.event <- list(PatientID=patient.id,
                        ...
                        study=study,
                        Name=name,
                        Fields = list(date=date, service=NA, name = NA, site = "metastatic", side = NA))
         good.records.found <- good.records.found + 1
         result[[good.records.found]] <- new.event
      }
   }} # for encEvent
  ...
  if(nrow(tbl.omfSub)>0){
   for(Event in 1:nrow(tbl.omfSub)){
      ...
      new.event <- list(PatientID=patient.id,
                        ...
                        study=study,
                        Name=name,
                        Fields = list(date=date, service=NA, name = Surgeryname, site = NA, side = side))
      good.records.found <- good.records.found + 1
      result[[good.records.found]] <- new.event
      }} # for encEvent
...
test_create.Procedure.record <- function()
{
    ...
    checkTrue(is.list(x))
    checkEquals(x[[1]], list(PatientID="TCGA.CS.6290", PtNum=1, study=study, Name="Procedure", Fields=list(date=NA, service=NA, name=NA, site="Supratentorial, Temporal Lobe: Cerebral Cortex", side="Left")))

    x <- create.Procedure.record("TCGA-HT-8564")
    checkEquals(names(x[[1]]), c("PatientID", "PtNum","study", "Name", "Fields"))
    checkEquals(names(x[[1]][["Fields"]]), c("date", "service", "name", "site", "side"))
    checkEquals(x[[1]], list(PatientID="TCGA.HT.8564", PtNum=188, study=study, Name="Procedure", Fields=list(date=NA, service=NA, name=NA, site="Supratentorial, Temporal Lobe", side="Left")))
    checkEquals(x[[2]], list(PatientID="TCGA.HT.8564", PtNum=188, study=study, Name="Procedure", Fields=list(date="04/30/2012", service=NA, name=NA, site="locoregional", side=NA)))
...
create.all.Procedure.records <- function(patient.ids)
{
   ...
   tbl.f1Sub  <- subset(tbl.f1,  bcr_patient_barcode %in% patient.ids & (new_tumor_event_surgery_days_to_loco != "[Not Available]" | new_tumor_event_surgery_days_to_met != "[Not Available]"))
   tbl.nteSub <- subset(tbl.nte, bcr_patient_barcode %in% patient.ids & (new_tumor_event_surgery_days_to_loco != "[Not Available]" | new_tumor_event_surgery_days_to_met != "[Not Available]"))
   tbl.omfSub <- subset(tbl.omf, bcr_patient_barcode %in% patient.ids & days_to_surgical_resection != "[Not Available]")
  ids <- unique(c(tbl.ptSub$brc_patient_barcode, tbl.f1Sub$bcr_patient_barcode,tbl.omfSub$bcr_patient_barcode,tbl.nteSub$bcr_patient_barcode ) )  
...
create.Absent.record <- function(patient.id)
{
   ...
   tbl.f1Sub  <- subset(tbl.f1,  bcr_patient_barcode==patient.id & new_tumor_event_dx_days_to != "[Not Available]" & (new_tumor_event_pharmaceutical_tx == "NO" | additional_radiation_therapy == "NO"))
  ...
  if(nrow(tbl.f1Sub)>0) { 
     f1 <- tbl.f1Sub[, c("new_tumor_event_dx_days_to", "additional_radiation_therapy", "new_tumor_event_pharmaceutical_tx")  ]
...
create.all.Absent.records <- function(patient.ids)
{
   ...
   tbl.f1Sub  <- subset(tbl.f1,  bcr_patient_barcode %in% patient.ids & new_tumor_event_dx_days_to != "[Not Available]" & (new_tumor_event_pharmaceutical_tx == "NO" | additional_radiation_therapy == "NO"))
