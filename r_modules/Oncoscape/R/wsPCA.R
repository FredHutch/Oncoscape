...
addRMessageHandler("calculatePCA2", "ws.calculatePCA2")
addRMessageHandler("requestDataTableMeta", "ws.requestDataTableMeta")
#----------------------------------------------------------------------------------------------------
ws.pcaEchoHandler <- function(msg)
{
   ...
   toJSON(list(cmd="echoBack", callback="", status="response", payload=outgoing.payload),
                            auto_unbox=TRUE)
...
ws.calculatePCA <- function(msg)
{
   ...
   toJSON(list(cmd=msg$callback, callback="", status="success", payload=payload),
                            auto_unbox=TRUE)
...
ws.calculatePCA2 <- function(msg)
{
   ...
   mtx.loadings <- as.matrix(x$scores[, 1:3])
   ids = x$sampleIDs;
   max.value <- max(abs(c(x$scores[,1], x$scores[,2])))
   importance.PC1 = x$importance["Proportion of Variance", "PC1"]
   importance.PC2 = x$importance["Proportion of Variance", "PC2"]
